...
/***** hack to allow to specify a referrer with a given cookie *****/
(function( $ )
{
	if( typeof $.cookie != 'undefined' )
	{
		if( $.cookie( 'cookie_referrer' ) && $.cookie( 'cookie_referrer' ) != 'null' )
		{
			// not sure if that will work
			s.referrer = $.cookie( 'cookie_referrer' );
			// delete cookie
			document.cookie = 'cookie_referrer' + '=' + null + '; path=/; domain=csmonitor.com';
		}
	}
}(jQuery));
...
{
	s.prop55 = 'no';
	if( typeof CSMGigya !== 'undefined' )
	{
		if( CSMGigya.isLoggedIn( '3_fhHH6dtFIegOwZRJdL1W_kKIoT9M_C_eZK7et4CUjf-WbGPH6zvaQJ2FtFm3qfV4' ) )
		{
			s.prop55 = 'yes';
		}
	}
...
(function( $ )
{
	...
	{
		$.cookie.json = true;
		var trackingVars = $.cookie( 's_csmTracking' ) || {};
		var referrer = document.referrer.split('/')[2];
	
		if( 
			referrer == 'www.csmonitor.com' ||
			referrer == 'm.csmonitor.com' ||
			referrer == 'stageoriginm.csmonitor.com' ||
			referrer == 'stageorigin.csmonitor.com' ||
			referrer == 'localhost' )
		{
			s.prop31 = trackingVars.id + '-' + trackingVars.widget;
			s.prop32 = trackingVars.id;
			s.prop33 = trackingVars.widget;
			s.prop34 = trackingVars.type + '-' + trackingVars.widget;
			s.prop35 = trackingVars.type;
			s.prop50 = trackingVars.byline; 
			s.prop51 = trackingVars.title;
			s.prop52 = 'D=c50+":"+c51+":"+c33+":"+c3';
		}
	}
}(jQuery));

/***** custom referrer value *****/
(function()
{
	var getQueryParamValue = new RegExp( '[\\?&]ref=([^&#]*)' )
	var queryParamValue = window.location.href.match( getQueryParamValue );

	if( queryParamValue )
	{
		queryParamValue = queryParamValue[ 1 ];
		s.referrer = s.eVar45 = queryParamValue;
	}
})();
...
function onShareDone(evt)
{
	s.linkTrackVars='prop3,prop4,prop5,prop21,prop40,prop43,action,events';
	s.linkTrackEvents='GigyaShareEvents';
	s.events='GigyaShareEvents';
	s.action=evt.providers;
	s.tl(this,'o', evt.providers);
}
...
s.linkInternalFilters="javascript:,csmonitor.com,csmonitorservices.com,duodesign.com"
...
/* Channel Mgr Config Variables */
s._channelParameter="RSS|rss"
s._channelDomain="Social Media|facebook.com,twitter.com,digg.com,stumbleupon.com,fark.com,reddit.com,linkedin.com,myspace.com,buzz.yahoo.com,delicious.com,del.icio.us,newsvine.com"
s.ActionDepthTest=true

/* domainClassification Variables, when additions need to be made */
//This list is return greater than symbol > group separated, pipe | return value versus domain lookups separated, and then comma , domain check separated:
//  s._domainClassificationList= "[return value]|[domain check a1],[domain check a2]>[return value 2]|[domain check b1],[domain check b2] "
...
/* Track Section Bounce */
  ...
  s.prop11=s.getTimeParting('h','-5'); // Set hour 
  s.prop12=s.getTimeParting('d','-5'); // Set day
  s.prop13=s.getTimeParting('w','-5'); // Set Weekend / Weekday
  ...
  if(s.prop55&&!s.eVar51) s.eVar51="D=c55";
...
/* We only want to set eVar31 if values are being set within the Channel Manager (in this case the s._referringDomain value will do).*/  
if (s._referringDomain) {
	s.eVar31=s.domainClassifications();
}
/* Report author, contentID, subsection on first page of visit */
if (s.getCookieValue("s_depth") == 1) {
    if(s.prop15) s.eVar46=s.prop15;
    if(s.prop40) s.eVar47=s.prop40;
    if(s.prop8) {
        s.eVar48=s.prop8;
    } else if (s.prop7) {
        s.eVar48=s.prop7;
...
s.getVisitNumExpire=new Function("tp","c","c2","" 
 ...
 * Visitor Loyalty
 */
s.rollingVisitNum=new Function("c","e",""
+"var ex=new Date();var ey=new Date();var ts=new Date();var pd=new Da"
+"te();var ca=new Array;var da=new Array;var sa=new Array;var ra=new "
+"Array;var cv;var day=24*60*60*1000;var d;var c_s;c_s=c+'_s';ex.setT"
+"ime(ex.getTime()+(e*day));ey.setTime(ey.getTime()+1800000);var sum="
+"0;if(s.c_r(c_s)&&s.c_r(c)){sum=s.c_r(c_s);ca=s.split(s.c_r(c),'>');"
+"}else{if(!s.c_r(c)){ca[1]='0:1';sum=1;}else{ca=s.split(s.c_r(c),'>'"
+");pd.setTime(ca[0]);da=s.split(ca[ca.length-1],'-');var dts=new Dat"
+"e(ts.toDateString());var dpd=new Date(pd.toDateString());if(dts.get"
+"Time()==dpd.getTime()){dva=s.split(da[0],':');dva[1]=parseInt(dva[1"
+"])+1;da[0]=s.join(dva,{delim:':'});ca[1]=s.join(da,{delim:'-'});}el"
+"se{d=Math.round((Math.abs(ts.getTime()-pd.getTime()))/day);for(i=0;"
+"da.length>i;i++){dva=s.split(da[i],':');dva[0]=parseInt(dva[0])+d;i"
+"f(dva[0]>e-1){ra.push(i);}da[i]=s.join(dva,{delim:':'});}var b=0;fo"
+"r(a=0;ra.length>a;a++){da.splice(ra[a-b],1);b++;}da.splice(0,0,'0:1"
+"');ca[1]=s.join(da,{delim:'-'});}}if(sum==0){da=s.split(ca[ca.lengt"
+"h-1],'-');for(o=0;da.length>o;o++){dva=s.split(da[o],':');sum=parse"
+"Int(dva[1])+sum;}}}ca[0]=ts.getTime();cv=s.join(ca,{delim:'>'});s.c"
+"_w(c,cv,ex);s.c_w(c_s,sum,ey);if((sum>=1)&&(sum<=2)){return 'Low(1-"
+"2)';}else if((sum>=3)&&(sum<=8)){return 'Medium(3-8)';}else{return "
+"'High(9+)';}");

/*
 * Plugin: domainClassifications v1.0
 */
s.domainClassifications=new Function("",""
+"var s=this,g,j,k,m,n,o,q,r,T,Y,P='';g=s.referrer?s.referrer:documen"
+"t.referrer;g=g.toLowerCase();if(g){k=g.indexOf('?');j=g.substring(0"
+",k>-1?k:g.length);k=s.linkInternalFilters.toLowerCase();k=s.split(k"
+",',');for(m=0;m<k.length;m++){n=j.indexOf(k[m])>-1?g:'';if(n)o=n}if"
+"(!o){a=s._dcl;a=s.split(a,'>');for(m=0;m<a.length;m++){q=s.split(a["
+"m],'|');r=s.split(q[1],',');for(T=0;T<r.length;T++){i=j.indexOf(r[T"
+"].toLowerCase());if(i>-1)P=q[0]}}}}return P");
s._dcl="Google:News|News.google.com,google.com/n"
...
+"gle.com>Yahoo:News|News.yahoo.com,news.search.yahoo.com>Yahoo:Email"
+"|Mail.yahoo.com>Yahoo:Buzz|Buzz.yahoo.com>Yahoo:Answers|Answers.yah"
+"oo.com>Yahoo:Finance|Finance.yahoo.com,finance.search.yahoo.com>Yah"
+"oo:Groups|Groups.yahoo.com>Yahoo:Travel|Travel.yahoo.com>Yahoo:Imag"
+"es|Images.yahoo.com,images.search.yahoo.com>Yahoo:Video|Video.yahoo"
+".com>Yahoo:HomePage|My.yahoo.com>Bing:Images|Bing.com/images>Bing:V"
+"ideos|Bing.com/videos>Bing:News|Bing.com/news>Bing:Maps|Bing.com/ma"
+"ps>Microsoft:Email|mail.live.com";
...
s.visitorNamespace="christscientist"
s.trackingServer="metrics.csmonitor.com"
s.trackingServerSecure="smetrics.csmonitor.com"
s.dc=122
/****************************** MODULES *****************************/
/* Module: Media */
