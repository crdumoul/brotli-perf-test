...
jQuery.url = function(){ 
		...
		$.each(params.replace(/\+/g, ' ').split('&'), function (j, v) {
			...
			if (param.length === 2) {
				...
				if (keys_last) {
					...
					// arrays are allowed.
					if (obj[key] != undefined && obj[key].constructor.toString().indexOf("Array") >= 0) {
	...
	function parse(url_str) {
		...
		*/
		...
		if(typeof(url_str) == "string" && url_str.indexOf("?") != -1){	
			
			if(url_str.indexOf("#") != -1){
				urlQuery = url_str.substr(url_str.indexOf("?") + 1, url_str.lastIndexOf("#") -1);
				urlAnchor = url_str.substr(url_str.lastIndexOf("#") + 1);
				
			}else{
				urlQuery = url_str.substr(url_str.indexOf("?") + 1);
			}
			url_str = url_str.substr(0, url_str.indexOf("?"));
		}else if (typeof(url_str) == "string"){
			if(url_str.indexOf("#") != -1){
				urlAnchor = url_str.substr(url_str.lastIndexOf("#") + 1);
				url_str = url_str.substr(url_str.lastIndexOf("#"));
			}		
		}		
		...
		if(typeof(urlQuery) != "undefined" && urlQuery != ""){
			uri[ "query" ] = urlQuery;
		}
		
		if(typeof(urlAnchor) != "undefined" && urlAnchor != ""){
			uri[ "anchor" ] = urlAnchor;
		}
