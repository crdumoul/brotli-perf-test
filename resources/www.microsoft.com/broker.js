...
if(/www\.microsoft\.com\/windows\/pc-selector/i.test(SR_url) ) {	
	...
	_sr_config = "broker-config_WinEmb.js";	
		...
		_sr_config = "broker-config_cle.js";
}else if(/mscom-switch-staging\.azurewebsites\.net\/en-us\/switch\/mac-surface/i.test(SR_url) ) {	
	_sr_builder = "builder-qInvite.js";	
}else if(/[\w\.]+\/en-us\/switch\/mac-surface/i.test(SR_url) ) {
	_sr_builder = "builder-qInvite.js";	
}
...
if(/iphone|ipad|ipod|android|opera mini|blackberry|bb10|mobile safari|windows (phone|ce)|iemobile|htc|nokia|mobile/i.test(navigator.userAgent) ){
	...
	}else if(/en-us\/default\.aspx\?comscore/i.test(SR_url) ){ 
		_sr_config = "broker-config_M.js"; _recruit4mobile=true;
		...
		return {
			...
			*/
			...
			start: function() {
				...
				}else if (/[\w\.]+\/zh-cn\/security/i.test(SR_url)) {
					 	//Disable recruitment for mobile
				}else {
          ...
          this.init();
			...
			loadBuilder: function() {
				...
				if(_sr_builder == "builder_Qbi.js" || _sr_builder == "builder-qInvite.js"){
