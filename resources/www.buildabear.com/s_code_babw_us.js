...
var isES = false;
if (window.location.toString().toLowerCase().indexOf("buildabear.es") >= 0){
	isES = true;
}

/*var s_account="babearcom";*/
...
s.linkInternalFilters="javascript:,buildabear.com";
...
function s_doPlugins(s) {
	...
	//external campaigns
	if (s.getQueryParam('sc_cid').length > 0){
		s.campaign=s.getQueryParam('sc_cid').toLowerCase();
	}
	if(s.getQueryParam('sc_ppd').length > 0){
		s.campaign=s.getQueryParam('sc_ppd').toLowerCase();
	}
	if(s.getQueryParam('sc_ipromo').length > 0){
		s.eVar14=s.getQueryParam('sc_ipromo').toLowerCase();
	}
	if(s.getQueryParam('sc_abc').length > 0){
		s.eVar18=s.getQueryParam('sc_abc').toLowerCase();
	}
	if(s.getQueryParam('sc_hpan').length > 0){
		s.eVar19=s.getQueryParam('sc_hpan').toLowerCase();
	}
	if(s.getQueryParam('sc_sran').length > 0){
		s.eVar20=s.getQueryParam('sc_sran').toLowerCase();
	}
	if(s.getQueryParam('sc_hpdr').length > 0){
		s.eVar23=s.getQueryParam('sc_hpdr').toLowerCase();
	}
	if(s.getQueryParam('sc_sct').length > 0){
		s.eVar25=s.getQueryParam('sc_sct').toLowerCase();
	}
	if(s.getQueryParam('sc_loginmethod').length > 0){
		s.eVar26=s.getQueryParam('sc_loginmethod').toLowerCase();
	}
	if(s.getQueryParam('sc_emailsignup').length > 0){
		s.eVar27=s.getQueryParam('sc_emailsignup').toLowerCase();
	}
	if(s.getQueryParam('sc_bvan').length > 0){
		s.eVar33=s.getQueryParam('sc_bvan').toLowerCase();
	}
	if(s.getQueryParam('entry').length > 0){
		s.prop13=s.getQueryParam('entry').toLowerCase();
	}
}
