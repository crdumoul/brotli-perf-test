...
/* Last Update: 9.30.2015.2014 DD*/
var s_account = "dtagdollar2,dtagglobal";
...
s.linkInternalFilters = "javascript:,gomez.com,prepaid.thrifty.co.uk,www.thriftycmla.com,cf.thrifty.com,www.dollar.de,www.dollarcanada.ca,www.dollar.co.uk,dollar.com"
...
/* Plugin Config */
s.usePlugins = true;
var s_doPluginsComplete;

function s_doPlugins(s) {
    /***********************BEGIN INTERNAL CAMPAIGN TRACKING************************************/
    if (!s.eVar2) {
        s.eVar2 = s.getQueryParam('intcmp');
        s.eVar2 = s.getValOnce(s.eVar2, "s_evar2", 0);
    }
    /***********************ENDINTERNAL CAMPAIGN TRACKING**************************************/

    s.campaign = s.getQueryParam('campid');

    /*Set eVar3 to capture clicks from Thrifty.com*/
    s.eVar3 = s.getQueryParam('xsrch');
    s.eVar3 = s.getValOnce(s.eVar3, "cname", 0);

    /* Set transactionID & prop25 to equal the purchaseID */
    s.transactionID = s.purchaseID;
    s.prop25 = s.purchaseID;

    /* Timeparting */
    s.prop8 = s.getTimeParting('h', '-6', new Date().getFullYear()); // Set hour 
    s.prop9 = s.getTimeParting('d', '-6', new Date().getFullYear()); // Set day
    s.prop10 = s.getTimeParting('w', '-6', new Date().getFullYear()); // Set Weekend / Weekday
    s.eVar8 = s.prop8;
    s.eVar9 = s.prop9;
    s.eVar10 = s.prop10;

    //Query Strings capture for Evar25 and Evar26
    s.eVar25 = s.getQueryParam('rmid');
    s.eVar26 = s.getQueryParam('rrid');

    /* Capture ttid value for 1:1*/
    s.eVar46 = s.getQueryParam('ttid');

    /* getPagename 2.0 */
    if (!s.pageType && !s.pageName)
        s.pageName = s.getPageName();

    /* set pageName to lower case*/
    if (s.pageName) {
        s.pageName = s.pageName.toLowerCase();
    }

    //Previous Page Name
    s.prop30 = s.getPreviousValue(s.pageName, 'gpv_p30', '');

    //event3 automation
    if ((s.prop30 == "home page") && (s.pagename == "rate shop")) {
        s.events = s.apl(s.events, 'event3', ',', 2);
    }
    if ((s.prop30 == "home page") && (s.pagename == "rate shop detail")) {
        s.events = s.apl(s.events, 'event3', ',', 2);
    }

    /* TNT to SC Integration */
    s.tnt = s.trackTNT();
    s.prop50 = s.tnt; // Copy to TNT var to prop 50 for use with vista

    /*Capture TNT PCID*/
    if (window.mboxFactoryDefault && typeof mboxFactoryDefault.getPCId == "function")
        s.eVar45 = mboxFactoryDefault.getPCId().getId();

    var ttid = "";
    ttid = s.getValOnce(s.getQueryParam('ttid'), 's_ttid', 0);

    if (ttid)
        s.events = s.apl(s.events, "event12", ",", 1);

    /* Doubleclick Plugin Call */
    s.partnerDFACheck("dfa_cookie", "dfaid");

    if (!s.events)
        s.events = "";

    //End Additional Tag Implementation

    s_doPluginsComplete = true;
    s.plugins = ""
}
s.doPlugins = s_doPlugins;

/********************** HELPER FUNCTION SECTION *********************/
/* You may insert any helper functions you wish to use here.        */
...
/*
* Plugin: getQueryParam 2.4 UPDATED
*/
...
/*
* Plugin: getPageName v2.1 - parse URL and return UPDATED
*/
...
/*
* Plugin: getValOnce_v1.1 UPDATED
*/
...
/*
* Plugin Utility: apl v1.1 UPDATED
*/
...
/*
* Utility Function: split v1.5 (JS 1.0 compatible) UPDATED
*/
...
/*
* Plugin Utility: Replace v1.0 UPDATED
*/
...
+ "')!=-1&&s.getQueryParam(q)=='')){s.referrer='';docu"
...
s.visitorNamespace = "dtag";
s.dc = 112;

/*** the following two lines are only used for first party cookies ***/
/*s.trackingServer="metric.dollar.com"
s.trackingServerSecure="metrics.dollar.com"*/
