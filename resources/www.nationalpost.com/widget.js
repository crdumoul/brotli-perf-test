...
var w3072_widget = {
	baseUrl: 'http://tools.prnewswire.com/en-us/live/3072/',
	...
	run_ajax: function(){
		...
		if ( navigator.appName == 'Microsoft Internet Explorer' ) {
			...
			ajax.onload=function(){
				document.getElementById('w3072_widget').innerHTML =  ajax.responseText;
				w3072_widget.format_dates();
	...
	format_dates: function(){
		...
		var dates = document.querySelectorAll('#w3072_widget .w3072_datetime.localized');
		for ( var i = 0; i < dates.length; i++ ){
			var formattedDate;
			var utcDate = dates[i].getAttribute('utcDate');
			var location = dates[i].getAttribute('location');
			var localDate = new Date( utcDate + ' UTC' );
			var ago = Math.floor( ( new Date() - localDate )/60000);
			if ( ago < 360 ) {
				formattedDate = moment( localDate ).fromNow(); //.ago( ago );
			} else {
				formattedDate = moment( localDate ).format(dateTimeFormat);
			}
