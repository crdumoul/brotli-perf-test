(function(e,c,a,g){var d="playstationMenu",f={animateHeightDelaySpeed:0,animateFade:0,animateFadeMobile:300,returnURLParamName:"postSignInURL",returnURL:c.location.href+"#signin-complete",returnLinkSelector:".return-url-link",postSignOutURLSelector:"postSignOutURL"};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.options=e.extend({},f,h);
this._name=d;
this.init()
}b.prototype.init=function(){var T=this.options,r=this.$element.find("ul.header-nav"),u=e("div.global-menu-container"),B=u.find("div.menu-wrapper"),A=B.find("div.menu-section"),H=[],C=this.$element.find("li.nav-link"),D=this.$element.find(".nav-content"),P=this.$element,O,ad=false,J=e(".has-secondary-nav").length,Y;
var ae=e.fn.getURLVar("debug");
ae=(ae==="nav")?true:false;
if(ae===true){e("body").addClass("debug-mode")
...
}}}if(Modernizr.csstransitions){B.css({"-webkit-transition":"height "+T.animateHeightDelaySpeed+"s linear","-moz-transition":"height "+T.animateHeightDelaySpeed+"s linear","-ms-transition":"height "+T.animateHeightDelaySpeed+"s linear","-o-transition":"height "+T.animateHeightDelaySpeed+"s linear",transition:"height "+T.animateHeightDelaySpeed+"s linear"})
}e(".visualrow, .customisablefeature, .avatar-content, .device-list, .visitusbar").each(function(o,ai){var ag=0,af=0,ah=e(this),aj=ah.text().replace(/\s+/,"");
if(aj==""){ah.closest(".menu-section-wrapper").next("div.frosted-border-vertical").remove();
ah.closest(".menu-section-wrapper").remove()
}});
var q=function(aj){var af=aj.find(".main-content"),an=PDC.Browser.getViewport();
if(an=="mobile"||an==""){af.css("width","");
return
}var ai=aj.find(".side-content .linksit"),ag=af.find(".linksit"),ak=ag.outerWidth()+parseInt(ag.css("padding-right")),ah=e(c).width(),am=(ai)?ai.outerWidth()+parseInt(ai.css("padding-left")):0,o=60;
var al=Math.floor((ah-(am+o))/ak);
if(al>ag.length){al=ag.length
}af.width(al*ak+"px")
};
var L=e("html"),aa=e(a.getElementById("curtainRight")),V=e(a.getElementById("curtainLeft")),P=e("#main-header"),v=e("#handle").height(),F=e(a.getElementById("frame"));
var n=aa.offset(),t=0,z=n.left,R=2,w=aa.outerWidth()+R,ab=60,W=e(c).width(),y=0,U="mobile-lock",I=false,M=false;
var N=new Hammer(a.getElementById("curtainRight")),x=new Hammer(a.getElementById("handle"));
var p=function(){W=e(c).width();
w=aa.outerWidth()+R;
t=aa.offset();
z=t.left
};
var k=function(){V.one("webkitTransitionEnd otransitionend oTransitionEnd msTransitionEnd transitionend",function(o){p();
F.removeClass("first")
})
};
var Q=function(){if(!aa.hasClass("active")){F.removeClass("noTrans");
aa.removeAttr("style");
k();
aa.addClass("active")
}};
var E=function(){k();
if(PDC.Browser.getViewport()=="mobile"){if(L.hasClass(U)){L.removeClass(U)
}else{L.addClass(U)
}}e("#curtainRight, #handle").removeAttr("style");
F.removeClass("noTrans");
F.toggleClass("mClosed mOpen")
};
var S=function(){if(P.hasClass("search-expanded")){P.removeClass("search-expanded")
}else{P.addClass("search-expanded")
}if(PDC.Browser.getViewport()=="mobile"){if(L.hasClass(U)){L.removeClass(U)
}else{L.addClass(U)
}}F.toggleClass("sClosed sOpen");
if(e("#frame").hasClass("sOpen")){e(".header-search input").focus()
}};
x.on("panstart",function(o){p()
});
x.on("pan",function(af){F.addClass("noTrans");
var o=z+af.deltaX;
if(o>=ab&&o<=w){aa.offset({left:o})
}if(o>ab){aa.removeClass("active")
}});
x.on("panend",function(o){p()
});
x.on("tap",function(o){if(!F.hasClass("noTrans")){aa.toggleClass("closed")
}Q();
F.removeClass("teach")
});
e("#menuBtn").on("click",function(af){E();
var o="hamburger";
if(F.hasClass("mClosed")){o="hamburger-close"
}PDC.SC.click("header",o,"button",null);
aa.removeClass("closed");
if(F.hasClass("sOpen")){S()
}af.preventDefault()
});
e(".search-toggle").on("click",function(o){if(F.hasClass("sClosed")){PDC.SC.click("header","search-banner","button",null)
}if(F.hasClass("mOpen")){E()
}S();
o.preventDefault()
});
e(a).ready(function(){if(PDC.Browser.getViewport()=="mobile"){var ah=PDC.Navigation.getHeight();
V.css("height","calc(100% - "+ah.curtainLeft+"px)");
B.css("height","calc(100% - "+ah.globalMenuWrapper+"px)")
}if(!J){var aj,ai=0,ak=5,af=e("nav"),ag=af.outerHeight();
e(c).scroll(function(al){aj=true
});
setInterval(function(){if(!e("html").hasClass("mobile-lock")&&PDC.Browser.getViewport()=="mobile"){if(aj){o();
aj=false
}}},250);
function o(){var al=e(a).scrollTop();
if(Math.abs(ai-al)<=ak){return
}if(al>ai&&al>ag){af.css("top","-"+af.outerHeight()+"px")
}else{if(al+e(c).height()<e(a).height()){af.css("top",0)
}}ai=al
}}});
function h(){var o=PDC.Browser.getViewport();
T.animateFade=(o=="mobile")?T.animateFadeMobile:T.animateFade;
if(PDC.Browser.getViewport()=="mobile"){e(".navlinks a").on("click",function(ag){var af=e(this);
af.removeClass("tap");
e(".navlinks li").removeClass("selected");
aa.removeClass("closed");
af.parent().addClass("selected");
Q();
if(I){F.removeClass("forcePrimary")
}e("#handle span").html(e(".selected .large-title").html());
F.removeClass("teach");
ag.preventDefault()
}).bind("touchstart",function(af){e(this).addClass("tap")
}).bind("touchend",function(af){if(M){e(this).removeClass("tap");
return
}e(this).removeClass("tap")
})
}else{L.removeClass("mobile-lock")
}C.each(function(af,aj){var ai=e(aj),ah=ai.find("div.menu-wrapper"),ag=e(c).width();
ah.css({display:"block"});
if(PDC.Browser.getViewport()!="mobile"){ah.width(ag);
if(ah.hasClass("home")){q(ah)
}}H[af]={height:ah.outerHeight(),section:ah.find("div.menu-section-content").clone(),classTags:ah.attr("class"),hasContent:!!ah.length};
ah.css({display:"none"})
})
}h();
if(Modernizr.csstransitions){Y=function(o,ah){var ag=o+"px";
if(PDC.Browser.getViewport()=="mobile"){var af=PDC.Navigation.getHeight();
ag="calc(100% - "+af.globalMenuWrapper+"px)"
}B.css({height:ag});
if(e.isFunction(ah)){c.setTimeout(ah,(T.animateHeightDelaySpeed*1000))
}}
}function Z(){var ag=false,ah=-1,af=e("div.touch-overlay");
function ai(){r.unbind("resize");
e.when(A.fadeOut(T.animateFade)).done(function(){Y(0,function(){e(this).hide();
e("li.selected-nav").removeClass("selected-nav")
})
});
ag=false
}function o(al,ak){var aj=PDC.Browser.getViewport();
T.animateFade=(aj=="mobile")?T.animateFadeMobile:T.animateFade;
ak.addClass("selected-nav");
if(aj!="mobile"){af.removeClass("visible");
r.bind("resize",function(){ai();
r.unbind("resize")
})
}else{r.unbind("resize")
}if(!ag){if(!H[al].hasContent){return true
}ag=true;
ah=al;
e.when(B.height(0).attr("class",H[al].classTags).show(),A.hide().remove("div.menu-section-content").html(H[al].section.clone())).done(function(){Y(H[al].height,function(){A.fadeIn(T.animateFade)
});
if(aj=="mobile"){if(F.hasClass("forcePrimary")){I=true
}}})
}else{if(al!==ah){ah=al;
e.when(A.fadeOut(T.animateFade)).done(function(){if(!H[al].hasContent){ai();
return false
}Y(H[al].height,function(){B.attr("class",H[al].classTags).show(),A.remove("div.menu-section-content").html(H[al].section.clone()).fadeIn(T.animateFade)
})
})
}}}D.each(function(ak,an){var am=e(this),al=false,aj=PDC.Browser.getViewport();
am.on("click",function(ao){if(am.parents().hasClass("nav-sitemap")){PDC.SC.click("header","sitemap","button",null)
}C.removeClass("selected-nav");
if((ag&&ak===ah)||!H[ak].hasContent){C.eq(ak).addClass("selected-nav");
if(aj!="mobile"){ai()
}}else{o(ak,am.parent());
if(aj!="mobile"){ao.preventDefault();
if(H[ak].hasContent){af.addClass("visible")
}else{af.removeClass("visible")
}}}})
});
if(PDC.Browser.getViewport()!="mobile"){af.bind("touchstart click",function(aj){aj.preventDefault();
ai();
af.removeClass("visible")
});
ai();
af.removeClass("visible");
e(c).resize(function(){if(O){c.clearTimeout(O)
}O=c.setTimeout(function(){ai();
af.removeClass("visible");
e("li.selected-nav").removeClass("selected-nav");
h()
},300)
});
e(c).on("orientationchange",function(){ai();
h()
})
}else{e(c).resize(function(){if(Modernizr.touch){if(PDC.Browser.getViewport()=="mobile"){if(F.hasClass("noTrans")){F.removeClass("noTrans");
aa.removeAttr("style")
}}}if(PDC.Browser.getViewport()=="mobile"){var aj=PDC.Navigation.getHeight();
V.css("height","calc(100% - "+aj.curtainLeft+"px)");
B.css("height","calc(100% - "+aj.globalMenuWrapper+"px)")
}});
e("body").on("touchmove",function(){M=true
}).on("touchstart",function(){M=false
})
}}function X(ai){var aj,ak,af=false,ag=-1;
function ah(){if(PDC.Browser.getViewport()=="mobile"){return
}if(!ae){if(!af){e("li.selected-nav").removeClass("selected-nav")
}else{ak=c.setTimeout(function(){e.when(A.fadeOut(T.animateFade)).done(function(){Y(0,function(){u.unbind("mouseenter").unbind("mouseleave");
e(this).hide();
e("li.selected-nav").removeClass("selected-nav")
})
});
...
}}}function o(an,am){e("li.selected-nav").removeClass("selected-nav");
am.addClass("selected-nav");
if(!af){if(!H[an].hasContent){return false
}aj=c.setTimeout(function(){af=true;
ag=an;
e.when(B.height(0).attr("class",H[an].classTags).show(),A.hide().remove("div.menu-section-content").html(H[an].section.clone())).done(function(){Y(H[an].height,function(){A.fadeIn(T.animateFade);
var ao=A.find(".menu-section-content"),ap=ao.find(".tier-1").attr("data-menu-javascript");
if(typeof ap!="undefined"&&e()[ap]){ao[ap]()
}e(c).bind("scroll",function aq(){u.unbind("mouseenter").unbind("mouseleave");
e.when(A.fadeOut(T.animateFade)).done(function(){Y(0,function(){e(this).hide();
e("li.selected-nav").removeClass("selected-nav")
})
});
af=false;
e(c).unbind("scroll",aq)
})
})
})
},250);
var al=PDC.Browser.getViewport();
if(al!="mobile"){u.bind("mouseenter",function(){e("li.selected-nav").removeClass("selected-nav");
e(D[ag]).parent().addClass("selected-nav");
c.clearTimeout(ak);
if(aj){c.clearTimeout(aj);
aj=null
}}).bind("mouseleave",function(){if(!aj){ah()
}})
}else{u.unbind("mouseenter");
u.unbind("mouseleave")
}}else{aj=c.setTimeout(function(){if(an!==ag){ag=an;
e.when(A.fadeOut(T.animateFade)).done(function(){if(!H[ag].hasContent){ah();
return false
}Y(H[an].height,function(){B.attr("class",H[an].classTags).show(),A.remove("div.menu-section-content").html(H[an].section.clone()).fadeIn(T.animateFade);
var ao=A.find(".menu-section-content"),ap=ao.find(".tier-1").attr("data-menu-javascript");
if(typeof ap!="undefined"&&e()[ap]){ao[ap]()
}})
})
...
}}D.each(function(an,ap){var ao=e(this),am=ao.parent(),al=PDC.Browser.getViewport();
if(al!="mobile"){ao.bind("mouseenter",function(){c.clearTimeout(aj);
aj=null;
c.clearTimeout(ak);
o(an,am)
}).bind("mouseleave",function(){if(!af){if(aj){c.clearTimeout(aj);
aj=null
}}ah()
})
}else{ao.unbind("mouseenter");
ao.unbind("mouseleave");
ao.bind("click",function(aq){c.clearTimeout(aj);
aj=null;
c.clearTimeout(ak);
o(an,am)
})
...
e(c).resize(function(){if(O){c.clearTimeout(O)
}O=c.setTimeout(function(){h()
},300)
});
if(ad==true){ai.trigger("mouseenter")
}}if(!Modernizr.touch){var K,G,l,m,ac;
e("body").one("mousemove.firstMove",function(o){K=o.target;
if(e(K).parents("ul").hasClass("header-nav")){G=e(K).parents("a").attr("href");
e("body").bind("mousemove.navOnly",function(af){m=af.target;
if(e(m).parents("ul").hasClass("header-nav")){ac=e(m).parents("a");
l=ac.attr("href");
if(G!=l){ad=true;
...
PDC.Navigation.getHeight=function(){var d=$("nav").height(),b=$("#handle").height(),c=$(".secondary-nav-container").height(),a=$(".secondary-nav-container").length;
if(a){d=d-c
}return{globalMenuWrapper:d+b,curtainLeft:d}
};
...
(function(e,c,a,g){var d="myProfile",f={pspImage:".psp-logo",avatarImage:".avatar-image",profileSection:".profile-section",profileId:"span.my-profile-id"};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.options=e.extend({},f,h);
this._defaults=f;
this._name=d;
this.init()
}b.prototype.init=function(){var l=PDC.Profile.isLoggedIn(),k=PDC.Region.getRegion(),h=this.$element.find(this.options.profileSection);
if(k&&l){this.addAvatar(h);
this.addProfileId(h)
}};
b.prototype.addAvatar=function(n){if(PDC.Profile){var k=PDC.Profile;
var o=k.getMediumAvatar();
var h=k.getPsPlus();
if(o){var l=n.find(this.options.avatarImage);
l.attr("src",o)
}if(h=="true"){var m=n.find(this.options.pspImage);
m.removeClass("hide")
}}};
b.prototype.addProfileId=function(k){if(PDC.Profile){var h=PDC.Profile.getOnlineID();
if(h!=null&&h){k.find(this.options.profileId).text(PDC.Profile.getOnlineID())
}}};
e.fn[d]=function(h){return this.each(function(){if(!e.data(this,"plugin_"+d)){e.data(this,"plugin_"+d,new b(this,h))
}})
}
...
(function(f,d,b,h){var e="navSignIn",g={};
...
function a(){var l=PDC.Profile;
var n=l.isLoggedIn();
var m=PDC.Region.getRegion();
if(m&&n){var k=f(".nav-sign-in-row").hide()
}else{f(".nav-sign-out-row").hide()
...
var PDC=PDC||{};
PDC.Func=PDC.Func||{};
PDC.Func.CM066=function(){var d=$(document.getElementById("footer-else-button"));
if(d.length===1){var c=d.parent(),b=d[0].href.split("#")[1],a=$(document.getElementById(b)),e="active";
if(a.length===1){d.on("click",function(f){f.preventDefault();
c.toggleClass(e);
a.stop(true,true).slideToggle("fast",function(){var g=a.is(":visible");
if(g){c.addClass(e)
}else{c.removeClass(e)
}})
})
}}$("#feedbackButtonSocialPod").on("click",function(){showWaypoint();
if(typeof s!=="undefined"){PDC.SC.click("socialpod","feedback overlay","image")
}});
$("#feedbackButtonFooterLinkRow").on("click",function(){showWaypoint();
if(typeof s!=="undefined"){PDC.SC.click("footer","feedback overlay","image")
}});
$("#feedbackButtonFooter").on("click",function(){showWaypoint();
if(typeof s!=="undefined"){PDC.SC.click("footer","feedback overlay","image")
}})
};
...
String.prototype.esc_campaign_sc=function(){return this.replace(/&[^;\s]+;/g,"").replace(/[^\w:_-]+/g," ").replace(/^\s+/g,"").replace(/\s+$/g,"").replace(/\s+/g," ").toLowerCase()
};
...
cid=cid[1].esc_campaign_sc();
if(PDC.SC.vars[consts.SMCID_COOKIE]==="emcid"){var cfunc=$.cookie?$.cookie:getCookie;
if(cfunc(COOKIE_ID)===cid&&cfunc(COOKIE_TYPE)===PDC.SC.vars[consts.SMCID_COOKIE]){cid=null;
PDC.SC.vars[consts.SMCID_COOKIE]=null
}}console.log("sc: pretest",PDC.isUserConsentedCookie());
if(PDC.isUserConsentedCookie()&&cid!=null){var host=".playstation.com";
...
if(cookieType!==consts.CAMPAIGN_ID&&cookieType!=="emcid"){cid=cfunc(COOKIE_ID);
...
}else{if(PDC.SC.vars.pageTemplate==="/apps/mypdc/template/publicprofilepage"){prop10="public-profile"
...
}else{if(PDC.SC.vars.pageTemplate==="/apps/mypdc/template/publicprofilepage"){sc.channel="public-profile";
sc.pageName="pdc:public-profile"
}else{if(title==="404"){sc.pageName="";
...
PDC.Useractivity.prepareVars();
PDC.Useractivity.fireEvent("pageview",sc);
...
$(window).on("sitecatalysImpression",function(){pdclog("sitecatalysImpression Fired");
PDC.SC.impression_creator()
})
};
PDC.SC.impression_creator=function(){var a=s.pageName.esc_sc_ns();
$("div").filter("[data-impression]").each(function(){var d=$(this);
var b=d.data("impression").componentCtx;
var f=d.data("impression").options||{};
var e=f.vars.list1;
f.vars.list1=f.vars.list1+":"+a;
var c=d.data("impression").entity||"";
pdclog("sitecatalysImpression call to sitecatalyst");
PDC.SC.click(b,e,c,f)
})
};
...
}var f=d?$(d).find("a, div[data-href]"):$("a, div[data-href]");
...
PDC.prod={"www.playstation.com":true,"playstation.com":true,"secure.us.playstation.com":true,"my.playstation.com":true};
...
}}else{if(d){if(!a.match(/pricespider.com/)){a=a+(a.indexOf("?")>-1?"&":"?")+"cid="+d
...
}}var y=$(d).prop("tagName");
if(y=="A"){h=d
}pdclog(["entity=",l,"tag=",y,"ele=",h]);
if(l=="image"&&y!="IMG"){d=$(d).find("img");
y=d.prop("tagName");
pdclog(["entity=",l,"tag=",y,"|round2"])
}if(y=="IMG"){if(v=="null"){v=$(d).attr("alt");
u="image";
if(!v&&d.attr("src")){v=PDC.SC.get_material_id(d.attr("src"))
}}}else{if(y=="A"){if(v=="null"){v=$(d).text();
pdclog("sc: click: A.linktitle="+v)
}var b=$(d).attr("class");
if(b&&b.indexOf("link-btn")>-1){u="button"
}}}}else{v=d
}if(u=="button"){PDC.SC.click_process_button(d,m)
}if(l=="priceSpiderButton"){var w=$(d).closest(".priceSpiderButton");
PDC.SC.click_process_priceSpiderbutton(w,m);
n=w.data("clientcontext")
}if(!l){l=u
}m._ctx=n;
m._linktitle=v;
m._entity=l;
...
PDC.Useractivity.fireEvent("click",m);
...
if(a.indexOf("psmedia.playstation.com")>-1||a.indexOf("media.playstation.com")>-1){pdclog(["href is scene7, not filling in placeholders: ",a]);
return
...
PDC.SC.click_process_priceSpiderbutton=function(b,c){pdclog("button processing");
c.vars=c.vars||{};
c.events=c.events||[];
try{var a=$(b).attr("class");
var h={};
var f=PDC.SC.click_extract_embedded_xmap(a,h);
var d=$(b);
h.sku=d.data("product-sku");
h.title=d.data("product-title");
h.preorder=h.title.indexOf("Pre")>-1;
c.events.push("event54");
if(h.preorder){c.events.push("event51")
}else{c.events.push("event32")
}var k=PDC.SC.vars.sku;
if(!k){k=h.sku?h.sku:h.upc
}c.vars.eVar20=c.vars.prop20=h.title.esc_sc().replace(/\s+/g,"-")+":"+k
}catch(g){pdclog("process_PriceSpider_button: "+g)
}};
...
var PDC=PDC||{};
PDC.Useractivity={_vars:{},MAPPINGS:{sitecat:{description:"SiteCatalyst",expr:new RegExp(/(.+)/),obj:{}},document:{description:"Document location object",expr:new RegExp(/(hash|host|hostname|href|origin|pathname|port|protocol|querystring)/),obj:{}},navigator:{description:"navigator object",obj:{}},query:{description:"Querystring parameters",expr:new RegExp(/(.+)/),obj:{}},page:{description:"Page Values",expr:new RegExp(/(.+)/),obj:{}}},_MAPPINGS:{},prepareVars:function(){var b=PDC.Useractivity;
b.MAPPINGS.sitecat.obj=s;
b.MAPPINGS.navigator.obj=navigator;
b.MAPPINGS.document.obj={hash:document.location.hash,host:document.location.host,hostname:document.location.hostname,href:document.location.href,origin:document.location.origin,pathname:document.location.pathname,port:document.location.port,protocol:document.location.protocol,querystring:document.location.search,referrer:document.referrer};
$("meta").each(function(){var f=$(this);
var e=f.attr("name");
var g=f.attr("content");
if(!b.MAPPINGS.page.obj[e]){b.MAPPINGS.page.obj[e]=g;
pdclog(["useractivity: prepare: page: ",e,g])
}});
var c=b.MAPPINGS.document.obj.querystring.split("&");
for(var a=0;
a<c.length;
a++){var d=c[a].split("=",2);
if(d[0]!=""){b.MAPPINGS.query.obj[d[0]]=d[1]
}}pdclog(["useractivity: prepare: query: ",b.MAPPINGS.query.obj]);
b._MAPPINGS.sitecat=b.MAPPINGS.sitecat.obj;
b._MAPPINGS.document=b.MAPPINGS.document.obj;
b._MAPPINGS.navigator=b.MAPPINGS.navigator.obj;
b._MAPPINGS.page=b.MAPPINGS.page.obj;
b._MAPPINGS.query=b.MAPPINGS.query.obj
},exprEval:function(m,b){var e=m.split(/\s*\+\s*/);
var p="";
for(var g=0;
g<e.length;
g++){var h=e[g];
if(h.charAt(0)=="'"){p+=h.substr(1,h.length-2)
}else{var c="null";
var d=[];
if(h.indexOf("|")>0){h=h.split("|",2);
d=h[1].split(",");
h=h[0]
}if(h.indexOf(":")>0){h=h.split(":",2);
c=h[1];
...
var l=a[0];
var n=a[1];
if(b[l]&&b[l][n]){c=b[l][n]
}if(d.length>0){for(var k=0;
k<d.length;
k++){var o=d[k];
if(PDC.Useractivity.EXPR_FILTERS[o]){c=PDC.Useractivity.EXPR_FILTERS[o](c)
}}}p+=c
}}return p
},EXPR_FILTERS:{lower:function(a){return a.toLowerCase()
},upper:function(a){return a.toUpperCase()
}},addStringFilter:function(name,func){if(typeof(func)=="string"){try{eval("func = "+func)
}catch(e){pdclog(["useractivity: addStringFilter: failed eval: ",func]);
return
}}if(typeof(func)=="function"){PDC.Useractivity.EXPR_FILTERS[name]=func;
pdclog(["useractivity: addStringFilter: added ",name,func])
}},_EVENTS:[],registerEvent:function(b,a){PDC.Useractivity._EVENTS.push({type:b,config:a})
},fireEvent:function(type,data){var collect={};
var map=PDC.Useractivity._MAPPINGS;
map.data=data;
for(var i=0;
i<PDC.Useractivity._EVENTS.length;
i++){var evt=PDC.Useractivity._EVENTS[i];
if(evt.type==type){if(evt.config.filter){var func=evt.config.filter;
if(typeof(func)=="string"){try{eval("func = "+func)
}catch(e){}}if(typeof(func)=="function"){if(!func(type,data)){continue
}}}for(var j=0;
j<evt.config.mappings.length;
j++){var pe=evt.config.mappings[j].split("=",2);
var prop=pe[0];
var expr=pe[1];
var val=PDC.Useractivity.exprEval(expr,map);
collect[prop]=val;
pdclog(["useractivity: fire",prop,expr,val])
}}}pdclog(["useractivity: collect",type,collect]);
PDC.Useractivity.record(type,collect)
},loadEvents:function(a){$.get(a,function(d){if(d.events){for(var c=0;
c<d.events.length;
c++){pdclog(["useractivity: loadEvents: ",d.events[c]]);
PDC.Useractivity.registerEvent(d.events[c].type,d.events[c]);
var e=d.events[c].stringFilterName;
var b=d.events[c].stringFilter;
if(e&&b){PDC.Useractivity.addStringFilter(e,b)
}}}},"json")
},record:function(b,d){if(!PDC.useractivityUrl||PDC.useractivityUrl==""){return
}var c=$.cookie("uaid");
if(c){d.visitor=c
}var a=$.cookie("userinfo");
if(a){d.userInfoCookie=a
}d.event_type=b;
$.post(PDC.useractivityUrl+"?type="+b,d,function(e){pdclog(["useractivity: record: callback",e]);
if(!c&&e.visitorId){$.cookie("uaid",e.visitorId,{domain:".playstation.com",path:"/",expires:365})
}},"json")
}};
PDC.Useractivity.loadEvents("/etc/pdc/ua-config.useractivity.js");
...
this.$element.one("click",".close",function(m){m.preventDefault();
var n=e("#main-header").find(".alert-bar");
h.$element.remove();
n.show();
if(n.length===0){h.$pageContentContainer.css("padding-top",h.nPageContentPadding+"px")
}else{var k=parseInt(h.nPageContentPadding)+parseInt(n.outerHeight());
h.$pageContentContainer.css("padding-top",k+"px")
}clearTimeout(h.timeoutId);
e(c).unbind("resize."+h.element.id);
if(PDC.Browser.getViewport()=="mobile"){var l=PDC.Navigation.getHeight();
e("#curtainLeft").css("height","calc(100% - "+l.curtainLeft+"px)");
e("div.menu-wrapper").css("height","calc(100% - "+l.globalMenuWrapper+"px)")
}})
};
...
if(PDC.Browser.getViewport()!="mobile"){this.pageContainerPadding+=this.$sonyBar.outerHeight()
...
e(".root").click(function(Q){if(!e(".reduced:visible").length){if(PDC.Browser.getViewport()=="mobile"){Q.preventDefault()
}var P=e(this),o=e(".nav-bg");
if(P.hasClass("expand")){P.removeClass("expand");
if(v.$navigation.hasClass("gotFixed")){}else{}o.removeClass("opaque_bg");
...
var S=0,R=P.$window.scrollTop(),o=P.$alertBar.is(":visible");
if(P.cookieMsgBarOffset){P.cookieMsgBarOffset=S=P.$cookieMsgBar.outerHeight()
}else{if(o){P.alertBarOffset=S=P.$alertBar.outerHeight()
}}if(S!=null&&S!=Q){P.alertBarOffset=S;
P.totalPadding=P.pageContainerPadding+P.alertBarOffset;
P.$pageContainer.css("padding-top",P.totalPadding+"px")
}if(R>=P.totalPadding){P.$navigation.stop().css("top",-(P.totalPadding)+"px");
...
}else{P.$navigation.stop().css("top","0");
P.$pageContainer.stop().css("paddingTop",(P.totalPadding-R)+"px");
...
(function(e,c,a,g){var d="alertBar",f={};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.options=e.extend({},f,h);
this._defaults=f;
...
var l=h.options;
this.$alertBarActionElement=h.$element.find(".action");
this.$alertBarMessageElement=h.$element.find(".psnIndicator-message");
this.$pageContentContainer=e(a.getElementById("page-content"));
this.nCurrentCookieMessageHeight=this.$element.outerHeight();
this.nPageContentPadding=this.$pageContentContainer.css("padding-top").split("px")[0];
this.nNewPageContentPadding=parseInt(this.nPageContentPadding)+parseInt(this.nCurrentCookieMessageHeight);
if(PDC.Browser.getViewport()!="mobile"){this.nNewPageContentPadding-=22
}var k=e(a.getElementById("main-header"));
$secondaryNavContainer=e(a.getElementById("main-header")).find(".secondary-nav-container"),sonyBarHeight=22;
if($secondaryNavContainer.length!=0&&l.psnIndicator!="true"){return
}if(l.psnIndicator!="true"){var n=a.cookie.split("DisableAlertBar=");
if(n.length!==2||n.pop().split(";").shift()!=="YES"){var m=k.find(".cookie-message");
this.addClickHandler();
if(!m.length){this.updateContentPadding()
}}}else{if(!getCookie("DisableIndicator")){h.setImageURL();
this.addClickHandler()
}}};
b.prototype.updateContentPadding=function(){var h=this;
this.nCurrentCookieMessageHeight=this.$element.outerHeight();
this.$pageContentContainer=e(a.getElementById("page-content"));
this.nPageContentPadding=this.$pageContentContainer.css("padding-top").split("px")[0];
this.nNewPageContentPadding=parseInt(this.nPageContentPadding)+parseInt(this.nCurrentCookieMessageHeight);
if(PDC.Browser.getViewport()!="mobile"){this.nNewPageContentPadding-=sonyBarHeight
}this.$pageContentContainer.css("padding-top",this.nNewPageContentPadding+"px")
};
b.prototype.addClickHandler=function(){var k=this;
var h=k.options;
this.$element.on("click",".close",function(m){m.preventDefault();
k.$element.remove();
if(PDC.isUserConsentedCookie()&&h.psnIndicator!="true"){setCookie("DisableAlertBar","YES")
}if(PDC.isUserConsentedCookie()&&h.psnIndicator=="true"){e.cookie("DisableIndicator","YES",{expiry:0,domain:".playstation.com",path:"/"})
}k.$pageContentContainer.css("padding-top","");
clearTimeout(k.timeoutId);
e(c).unbind("resize."+k.element.id);
if(PDC.Browser.getViewport()=="mobile"){var l=PDC.Navigation.getHeight();
e("#curtainLeft").css("height","calc(100% - "+l.curtainLeft+"px)");
e("div.menu-wrapper").css("height","calc(100% - "+l.globalMenuWrapper+"px)")
}})
};
b.prototype.setImageURL=function(){var h=this;
var k=h.options;
e.ajax({url:k.psnIndicatorServiceUrl,dataType:"json",cache:false,async:true,crossDomain:true}).done(function(l){pdclog(l);
if(typeof l!=="undefined"&&l!==null){h.$alertBarActionElement.addClass("notice-icon");
h.$alertBarActionElement.attr("style","background-image: url('"+l.iconUrl+"')");
h.$alertBarMessageElement.attr("href","http://status.playstation.com/"+PDC.Locale.getLocale());
if(l.statusType=="OK"){h.$element.remove()
}else{if(l.statusType.indexOf("Outage")>-1||l.statusType.indexOf("Down")>-1){h.$alertBarMessageElement.text(Adobe.I18n.get("Playstation Network is currently experiencing issues"));
e("<div>").html("&gt;").addClass("chev hide-text").appendTo(h.$alertBarMessageElement);
h.$element.show();
h.nNewPageContentPadding=parseInt(h.nPageContentPadding)+parseInt(h.nCurrentCookieMessageHeight)-22;
h.$pageContentContainer.css("padding-top",h.nNewPageContentPadding+"px")
}else{if(l.statusType==="Maintenance"||l.statusType==="Degraded"){h.$alertBarMessageElement.text(Adobe.I18n.get("Playstation Network is currently under maintenance"));
e("<div>").html("&gt;").addClass("chev hide-text").appendTo(h.$alertBarMessageElement);
h.$element.show();
h.nNewPageContentPadding=parseInt(h.nPageContentPadding)+parseInt(h.nCurrentCookieMessageHeight)-22;
h.$pageContentContainer.css("padding-top",h.nNewPageContentPadding+"px")
...
(function(e,c,a,g){var d="youtubeCarousel",f={videogroupIDs:"",videoIDs:"",templateId:"",onloadvideo:"",vidtemplateId:"",desktopCount:6,tabletCount:5,mobileCount:4,position:"horizontal"};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.options=e.extend({},f,h);
this._defaults=f;
...
this.$container=this.$element.find(".youtubePlayerContainer");
this.$imageOverride=this.$element.find(".pseudoFrame");
this.$playButton=this.$element.find(".playButton");
this.$pseudoFrame=this.$element.find(".pseudoFrame");
this.init()
}b.prototype.init=function(){var k=this;
var n=k.options.videogroupIDs;
var l=k.options.videoIDs;
var m=k.options.onloadvideo;
var h=k.$pseudoFrame.hasClass("imageOverride");
if(!k.isBlank(n)){e.getJSON("https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=50&playlistId="+n+"&key=AIzaSyD_v2XHR1lBThUSebq5rKY2v8OgLX42JPc&alt=json&callback=?",function(p){var q="";
var o=p.items.length-1;
e.each(p.items,function(r,t){q=q+"<div class='slimgrid'><div class='backGround'><a class='tile-image'   href='javascript:void(0)' data-videoID='"+t.snippet.resourceId.videoId+"' ><img  src='//img.youtube.com/vi/"+t.snippet.resourceId.videoId+"/hqdefault.jpg' class='ytImage' /></a></div></div>"
});
k.$carousalBlock.append(q);
if(!k.isBlank(l)){k.appendIndividualVideos()
}if(k.isBlank(m)){m=p.items[0].snippet.resourceId.videoId
}k.addonscarousal();
k.loadyoutubePlayer(m,k.$container,h,"0");
e.each(k.$carousalBlock.find(".tile-image"),function(r,t){e(t).on("click",function(v){var u=e(this).data("videoid");
k.loadyoutubePlayer(u,k.$container,false,"0")
})
})
})
}else{if(!k.isBlank(l)){k.appendIndividualVideos();
k.addonscarousal();
if(k.isBlank(m)){m=l.split(",")[0]
}k.loadyoutubePlayer(m,k.$container,h,"0");
e.each(k.$carousalBlock.find(".tile-image"),function(o,p){e(p).on("click",function(r){var q=e(this).data("videoid");
k.loadyoutubePlayer(q,k.$container,false,"0")
})
})
}}e.getJSON("https://www.googleapis.com/youtube/v3/videos?part=snippet&id=&key=AIzaSyD_v2XHR1lBThUSebq5rKY2v8OgLX42JPc&alt=json&callback=?",function(o){});
k.$playButton.on("click",function(o){k.loadyoutubePlayer(m,k.$container,false,"1")
});
k.$playButton.removeClass("hide")
};
b.prototype.appendIndividualVideos=function(){var h=this;
var l="";
var k=h.options.videoIDs.split(",");
var m="";
e.each(k,function(n,o){m=m+"<div class='slimgrid'><div class='backGround'><a class='tile-image'   href='javascript:void(0)' data-videoID='"+o+"' ><img  src='//img.youtube.com/vi/"+o+"/hqdefault.jpg' class='ytImage' /></a></div></div>"
});
h.$carousalBlock.append(m)
};
b.prototype.loadyoutubePlayer=function(k,l,n,q){var m=this;
if(n){e(l).hide()
}else{m.$pseudoFrame.hide();
e(l).show()
}var p={};
p.videoId=k;
p.autoPlay=q;
...
p.width="100%";
p.height="100%";
if(swfobject.hasFlashPlayerVersion("9.0.2")){var o=new psplayer.VideoPlayer(l.prop("id"),p)
}else{h(l)
}function h(r){r.html('<iframe width="100%" height="100%" src="//www.youtube.com/embed/'+p.videoId+"?autoplay="+p.autoPlay+'&rel=0&showinfo=0&autohide=1&iv_load_policy=3" frameborder="0" allowfullscreen></iframe>')
}};
b.prototype.isBlank=function(h){return(!h||/^\s*$/.test(h))
};
b.prototype.addonscarousal=function(){var h=this;
var k=false;
if(h.options.position==="vertical"){k=true
}h.$carousalBlock.slick({slidesToShow:h.options.desktopCount,slidesToScroll:h.options.desktopCount,dots:false,infinite:true,vertical:k,verticalSwiping:k,responsive:[{breakpoint:1024,settings:{slidesToShow:h.options.desktopCount,slidesToScroll:h.options.desktopCount}},{breakpoint:600,settings:{slidesToShow:h.options.desktopCount,slidesToScroll:h.options.desktopCount}}]})
};
e.fn[d]=function(h){return this.each(function(){if(!e.data(this,"plugin_"+d)){e.data(this,"plugin_"+d,new b(this,h))
}})
}
...
(function(e,c,a,g){var d="trophytips",f={trophyUrl:"//stagea.us.playstation.com/playstation/psn/review/trophy/details",reviewUrl:"//qa.io.playstation.com/playstation/psn/review",updateReviewUrl:"//qa.io.playstation.com/playstation/psn/review",avatarDataUrl:"//io.playstation.com/playstation/psn/profile/avatar/",reportURL:"//qa.io.playstation.com/playstation/psn/review/report",allGameTrophies:null};
function b(l,k){var h=this;
h.element=l;
h.$element=e(h.element);
h.$commentWrapper=h.$element.find(".trophyListWrapper");
h.options=e.extend({},f,k);
h._defaults=f;
h._name=d;
h.$elementParent=h.$element.parent();
this.loggedInOrNot=null;
this.reportText="";
this.htmlReportAbuse="";
this.init()
}b.prototype.init=function(){var k=this;
var l=k.options;
k.signedInOrNot();
k.fetchData();
sessionStorage.setItem("compareTrophiesVars","gid="+k.options.npcommid);
var h=k.$commentWrapper.data("href");
e.ajax({url:h,type:"GET"}).done(function(m){k.htmlReportAbuse=m
})
};
b.prototype.signedInOrNot=function(){var h=this;
h.loggedInOrNot=PDC.Profile.isLoggedIn();
if(h.loggedInOrNot){h.loggedInHandle=PDC.Profile.getOnlineID();
h.avatarURL=PDC.Profile.getMediumAvatar();
h.loggedInOrNot="true"
}else{h.loggedInOrNot="false"
}};
b.prototype.fetchData=function(){var h=this;
var k=h.options;
h.$ajaxPreloader=e("<div>").attr({"class":"ajax-loader"}).appendTo(".compWrapper");
e.ajax({url:k.trophyUrl,dataType:"json",data:{np_comm_id:h.options.npcommid,online_id:h.loggedInHandle},}).done(function(l){h.trophyData=l;
h.allGameTrophies=h.trophyData.trophies.slice(0);
h.$element.find(".ajax-loader").removeClass("ajax-loader");
h.buildDom()
}).fail(function(n,m,l){h.$element.addClass("hide");
h.allGameTrophies=null
})
};
b.prototype.buildDom=function(){var h=this;
var k=h.options;
var l=h.allGameTrophies;
if(l){if(h.$ajaxPreloader){h.$ajaxPreloader.remove();
h.$element.find(".compWrapper").css({height:"",position:""})
}h.buildTrophyList();
h.submitTip();
h.tipArrowToggle()
}};
b.prototype.reportingAbuse=function(n,h){var l=this;
var m=l.options;
var k={review_id:h,online_id:l.loggedInHandle};
if((n).attr("id")=="reportIt"){n.addClass("hide");
n.siblings("#reportedIt").removeClass("hide");
e.ajax({url:m.reportURL,type:"POST",data:k}).done(function(){})
}};
b.prototype.submitTip=function(){self=this;
var h=self.options;
self.$submit=self.$element.find(".tipSubmit");
self.$submit.css("background-color","rgb(224, 224, 224)");
self.$submit.prev("textarea").on("keyup",function(){if(e(this).val()==""){e(this).next(".tipSubmit").css("background-color","rgb(224, 224, 224)")
}else{e(this).next(".tipSubmit").css("background-color","#002bff")
}});
self.$submit.on("click",function(){var k=e(this).prev("textarea").val();
var o=e(this).prev("textarea");
var p=e(this).closest(".trophyTipandLogInDIv").attr("class");
p=p.slice(21);
p=parseInt(p);
var n=e(this).closest(".trophyTipandLogInDIv");
var l=e(this).closest(".trophyTipandLogInDIv").prev(".trophyListContainer").find(".contentTip");
var m=new Date();
var q=m.getDate()+"/"+(m.getMonth()+1)+"/"+m.getFullYear()+" @ "+m.getHours()+":"+m.getMinutes()+":"+m.getSeconds();
if(k!=""){e.ajax({url:h.updateReviewUrl,type:"POST",data:{np_comm_id:self.options.npcommid,entity_type:"trophy",entity_id:p,online_id:self.loggedInHandle,product_id:self.options.psnId,review_text:k,country:"us",locale:"en"}}).done(function(x){l.find(".contentTipNumber").css("display","block");
var C=parseInt(l.find(".numberOfComments").text());
if(isNaN(C)){C=1
}else{C=C+1
}l.find(".numberOfComments").text(C);
if(C>1){l.find(".contentTipNumber .moreThankOne").css("display","inline")
}l.find(".contentTipText").css("display","none");
var z=e("<div>").attr({"class":"trophyTip"}).prependTo(n);
var r=e("<table>").attr({"class":"trophyTipTable"}).appendTo(z);
var u=e("<tr>").attr({"class":"trophyTipRow"}).appendTo(r);
var y=e("<td>").attr({"class":"trophyTipperPic",}).appendTo(u);
var w=e("<img>").attr({}).attr("src",self.avatarURL).appendTo(y);
var A=e("<td>").attr({"class":"trophyTipperData"}).appendTo(u);
var t=e("<p>").attr({"class":"trophyTipperName"}).text(self.loggedInHandle).appendTo(A);
var v=e("<p>").attr({"class":"trophyTipperCommentDate"}).text(q).appendTo(A);
var B=e("<p>").attr({"class":"trophyTipperComment"}).text(k).appendTo(A);
o.val("")
})
}})
};
b.prototype.tipArrowToggle=function(){var h=this;
h.$element=e(h.element);
var k=h.options;
h.$element.find(".contentTipArrowDown").on("click",function(){var l=e(this).closest(".trophyListContainer").next(".trophyTipandLogInDIv").find(".trophyTip");
e(this).toggleClass("uparrow");
e(l).each(function(m,o){var p=e(o).find(".trophyTipperName").text();
var n=e(o).closest(".trophyTipandLogInDIv").attr("class");
h.getHandleURL(p,n)
});
e(this).closest(".trophyListContainer").next(".trophyTipandLogInDIv").slideToggle("slow")
})
};
b.prototype.filterTrophies=function(){var m=this;
function l(p){if("type" in p&&p.type=="PLATINUM"){return true
}}function n(p){if("type" in p&&p.type=="GOLD"){return true
}}function h(p){if("type" in p&&p.type=="SILVER"){return true
}}function o(p){if("type" in p&&p.type=="BRONZE"){return true
}}function k(p){if("type" in p&&p.type=="HIDDEN"){return true
}}m.arrayP=m.allGameTrophies.filter(l);
m.arrayG=m.allGameTrophies.filter(n);
m.arrayS=m.allGameTrophies.filter(h);
m.arrayB=m.allGameTrophies.filter(o);
m.arrayH=m.allGameTrophies.filter(k)
};
b.prototype.filterWonTrophies=function(){var m=this;
m.$platinum=m.$element.find(".platinum-trophy");
m.$gold=m.$element.find(".gold-trophy");
m.$silver=m.$element.find(".silver-trophy");
m.$bronze=m.$element.find(".bronze-trophy");
function l(p){if("type" in p&&p.type=="PLATINUM"&&p.earnedTime!=null){return true
}}function n(p){if("type" in p&&p.type=="GOLD"&&p.earnedTime!=null){return true
}}function h(p){if("type" in p&&p.type=="SILVER"&&p.earnedTime!=null){return true
}}function o(p){if("type" in p&&p.type=="BRONZE"&&p.earnedTime!=null){return true
}}function k(p){if("type" in p&&p.type=="HIDDEN"&&p.earnedTime!=null){return true
}}m.arrayPEarned=m.allGameTrophies.filter(l);
m.arrayGEarned=m.allGameTrophies.filter(n);
m.arraySEarned=m.allGameTrophies.filter(h);
m.arrayBEarned=m.allGameTrophies.filter(o);
m.arrayHEarned=m.allGameTrophies.filter(k);
m.totalTrophiesWon=m.arrayPEarned.length+m.arrayGEarned.length+m.arraySEarned.length+m.arrayBEarned.length+m.arrayHEarned.length;
e(m.$element).find(".totalTrophies > .earnedTrophies").text(m.totalTrophiesWon);
m.$platinum.find(".signedinWonTrophies").text(m.arrayPEarned.length);
m.$gold.find(".signedinWonTrophies").text(m.arrayGEarned.length);
m.$silver.find(".signedinWonTrophies").text(m.arraySEarned.length);
m.$bronze.find(".signedinWonTrophies").text(m.arrayBEarned.length)
};
b.prototype.getHandleURL=function(l,k){var h=this;
h.$trophyTips=h.$element.find(".trophyTip");
k=k.slice(21);
e.ajax({type:"GET",url:h.options.avatarDataUrl+l,dataType:"json"}).done(function(m){h.$trophyTips.find("."+l+" img").css("background-image",'url("'+m.avatarUrl+'")')
})
};
b.prototype.fetchTrophyComments=function(m){var h=this;
var k=h.options;
var l=h.loggedInOrNot=="true"?{entity_type:"trophy",entity_id:m,online_id:h.loggedInHandle}:{entity_type:"trophy",entity_id:m};
e.ajax({url:k.reviewUrl,type:"GET",dataType:"JSON",data:l}).done(function(n){h.trophyTipsData=n;
h.numberOfComments=h.trophyTipsData.length;
if(h.numberOfComments==1){e("."+m).prev(".trophyListContainer").find(".contentTipNumber").css("display","block");
e("."+m).prev(".trophyListContainer").find(".moreThankOne, .contentTipText").addClass("hide")
}else{if(h.numberOfComments>1){e("."+m).prev(".trophyListContainer").find(".contentTipNumber").css("display","block");
e("."+m).prev(".trophyListContainer").find(".contentTipText").addClass("hide")
}}e("."+m).prev(".trophyListContainer").find(".numberOfComments").text(h.numberOfComments);
e(h.trophyTipsData).each(function(v,A){var w=e("<div>").attr({"class":"trophyTip","data-rid":A.id}).prependTo("."+m);
var o=e("<table>").attr({"class":"trophyTipTable"}).appendTo(w);
var q=e("<tr>").attr({"class":"trophyTipRow"}).appendTo(o);
var z=q.append("<span id='reportIt' class='hide trophyTipReportAbuse'>"+Adobe.I18n.get("Report")+"</span><span id='reportedIt' class='hide trophyTipReportAbuse'>"+Adobe.I18n.get("Reported")+"</span><span id='logInToReport' class='hide trophyTipReportAbuse'>"+Adobe.I18n.get("Log In to report")+"</span>");
z.children("span#reportIt").on("click",function(){var B=e(this);
var C=e(this).closest(".trophyTip").data("rid");
C=parseInt(C);
h.reportingAbuse(B,C)
});
if(h.loggedInOrNot=="false"){z.children("#logInToReport").removeClass("hide")
}else{if(A.isReported=="0"){z.children("#reportIt").removeClass("hide")
}else{if(A.isReported=="1"){z.children("#reportedIt").removeClass("hide")
}}}var u=e("<td>").attr({"class":"trophyTipperPic "+A.handle}).appendTo(q);
var t=e("<img>").attr({}).appendTo(u);
var x=e("<td>").attr({"class":"trophyTipperData"}).appendTo(q);
var p=e("<p>").attr({"class":"trophyTipperName"}).text(A.handle).appendTo(x);
var r=e("<p>").attr({"class":"trophyTipperCommentDate"}).text(A.createdDate).appendTo(x);
var y=e("<p>").attr({"class":"trophyTipperComment"}).text(A.reviewText).appendTo(x)
})
}).fail(function(p,o,n){h.trophytipsData=null
})
};
b.prototype.buildTrophyList=function(){var t=this;
t.$element=e(t.element);
var q=t.options;
var l=t.allGameTrophies;
var m=Object.keys(l).length;
t.filterTrophies();
var r=t.arrayP.length;
var o=t.arrayG.length;
var n=t.arrayS.length;
var h=t.arrayB.length;
var p=t.arrayH.length;
t.trophiesInOrder=t.arrayP.concat(t.arrayG,t.arrayS,t.arrayB,t.arrayH);
var k=t.trophiesInOrder.length;
t.trophyId=[];
t.$element.find(".totalTrophiesAvail").text(l.length);
t.$element.find(".platinum-trophy span.trophiesToBeWon").text(r);
t.$element.find(".gold-trophy span.trophiesToBeWon").text(o);
t.$element.find(".silver-trophy span.trophiesToBeWon").text(n);
t.$element.find(".bronze-trophy span.trophiesToBeWon").text(h);
t.signInURL=t.$element.find("a.manual-other").attr("href");
t.trophiesInOrder=t.trophiesInOrder.slice(0);
e(t.trophiesInOrder).each(function(P,S){t.trophyId[P]=S.trophyId;
var v=e("<div>").attr({"class":"trophyListContainer"}).appendTo(".trophyListWrapper");
var U=e("<div>").attr({"class":"trophyDetailList"}).appendTo(v);
var N=e("<table>").attr({"class":"contentTable"}).appendTo(U);
var G=e("<tr>").attr({"class":"contentRow"}).appendTo(N);
var E=e("<td>").attr({"class":"contentTrophyImage",width:"8%",}).appendTo(G);
var H=e("<a>").attr({style:"background-image: url("+S.image+")"}).appendTo(E);
var M=e("<td>").attr({"class":"contentSpace",width:"1%"}).appendTo(G);
var T=e("<td>").attr({"class":"contentTrophyTypeImage",width:"5%"}).appendTo(G);
if(S.type=="PLATINUM"){var J="/etc/designs/pdc/clientlibs_base/images/TrophyTips/trophy_platinum.png"
}else{if(S.type=="GOLD"){var J="/etc/designs/pdc/clientlibs_base/images/TrophyTips/trophy_gold.png"
}else{if(S.type=="SILVER"){var J="/etc/designs/pdc/clientlibs_base/images/TrophyTips/trophy_silver.png"
}else{if(S.type=="BRONZE"){var J="/etc/designs/pdc/clientlibs_base/images/TrophyTips/trophy_bronze.png"
}else{if(S.type=="HIDDEN"){var J="/etc/designs/pdc/clientlibs_base/images/TrophyTips/trophy_icon.png"
}}}}}if(typeof S.type!=="undefined"&&S.type!==null&&S.type!==""){T.addClass(S.type.toLowerCase())
}var w=e("<img>").attr({src:J}).appendTo(T);
var O=e("<td>").attr({"class":"contentTrophyName",width:"57%"}).appendTo(G);
var x=e("<div>").attr({"class":"contentTrophyFullName yourTrophiesBlue",}).text(S.name).appendTo(O);
var x=e("<div>").attr({"class":"contentTrophyDesc",}).text(S.detail).appendTo(O);
var Q=e("<td>").attr({"class":"contentTip",width:"13%"}).appendTo(G);
var z=e("<p>").attr({"class":"contentTipNumber",style:"display:none"}).html("<span class='numberOfComments'></span> comment<span class='moreThankOne'>s</span>").appendTo(Q);
var u=e("<p>").attr({"class":"contentTipText"}).text(Adobe.I18n.get("Be the first to add a tip!")).appendTo(Q);
var A=e("<td>").attr({"class":"contentTipArrow",width:"3.5%"}).appendTo(G);
var I=e("<a>").attr({"class":"contentTipArrowDown downarrow"}).appendTo(A);
var M=e("<td>").attr({"class":"contentSpace",width:"4%"}).appendTo(G);
var C=e("<td>").attr({"class":"contentTrophyAcquired",width:"8%"}).appendTo(G);
if(t.loggedInOrNot=="false"){t.$element.find(".contentTrophyAcquired").css("background-color","#ffffff");
if(e(a).width<560){t.$element.find(".contentTrophyAcquired").css("background","rgba(0, 0, 0, 0) linear-gradient(to bottom, #ffffff 0%, #ffffff 40%, #eee 40%, #eee 100%)")
}}else{t.$element.find(".contentTrophyAcquired").css("background-color","#e0e0e0");
if(e(a).width<560){t.$element.find(".contentTrophyAcquired").css("background","rgba(0, 0, 0, 0) linear-gradient(to bottom, #e0e0e0 0%, #e0e0e0 40%, #eee 40%, #eee 100%)")
}}var F=e("<a>").attr({"class":"contentTrophyAcquiredAnchor"}).appendTo(C);
if(t.loggedInOrNot=="false"){F.css("background-image","url(/etc/designs/pdc/clientlibs_base/images/TrophyTips/trophy_icon.png)")
}else{if(S.earnedTime==null){F.css("background-image","url(/etc/designs/pdc/clientlibs_base/images/TrophyTips/trophy_icon.png)")
}else{F.css("background-image","url("+J+")")
}}t.$trophyTipandLogInDIv=e("<div>").attr({"class":"trophyTipandLogInDIv "+S.trophyId,style:"display: none"}).appendTo(".trophyListWrapper");
t.fetchTrophyComments(S.trophyId);
var D=e("<div>").attr({"class":"trophyTipListContainer signInorNot",}).appendTo(t.$trophyTipandLogInDIv);
var R=e("<div>").attr({"class":"trophyTipDetailList signInorNotBlock",}).appendTo(D);
(R).append("<p class='notLoggedIn'><a class='signInURL'>"+Adobe.I18n.get("SIGN IN")+"</a>"+Adobe.I18n.get(" to add your trophy tip.")+"</p>");
R.find(".signInURL").attr("href",t.signInURL);
var K=e("<div>").attr({"class":"LoggedIn"}).appendTo(R);
var y=e("<textarea>").attr({placeholder:Adobe.I18n.get("Submit your trophy tip here.")}).appendTo(K);
var L=e("<a>").attr({"class":"tipSubmit blue-btn"}).text("Submit").appendTo(K);
var B=e("<span>").attr({}).appendTo(L)
});
if(t.loggedInOrNot=="false"){t.$element.find(".earnedTrophies, .trophyForwardSlash, .LoggedIn, .signedInTextV, .LoggedIn, .compareTrophies").addClass("hide")
}else{t.$element.find(".signText, .notLoggedIn, .signInPlease").addClass("hide");
t.$element.find(".LoggedIn").css("display","block");
if(e(a).width()<560){t.$element.find(".mobile-only").addClass("hide")
}t.filterWonTrophies()
}};
e.fn[d]=function(h){return this.each(function(){if(!e.data(this,"plugin_"+d)){e.data(this,"plugin_"+d,new b(this,h))
}})
...
(function(e,c,a,f){var d="gigyaUserAction";
defaults={shareImageEnabled:true,defaultSharableImageURL:"https://media.playstation.com/is/image/SCEA/playstation-logo-imageblock-us-10mar15?$Icon$",shareType:"facebook-share"};
function b(l,k){var g=this;
g.element=l;
g.$element=e(g.element);
g.options=e.extend({},defaults,k);
g._defaults=defaults;
g._name=d;
var h=g.options;
if(typeof gigya!=="undefined"){g.init()
}}b.prototype.init=function(){var g=this;
g.showShareUI()
};
b.prototype.showShareUI=function(){var q=this;
var n=new gigya.socialize.UserAction();
var k=c.location.href;
var p=a.title;
n.setTitle(p);
var l="Playstation.com";
if(e("meta[property='og:description']").length){if(e("meta[property='og:description']").attr("content")!==""){l=e("meta[property='og:description']").attr("content")
}}n.setDescription(l);
n.setLinkBack(k);
if(q.options.shareImageEnabled){var m=q.options.defaultSharableImageURL;
if(e("meta[property='og:image']").length){if(e("meta[property='og:image']").attr("content")!==""){m=e("meta[property='og:image']").attr("content")
}else{e("meta[property='og:image']").attr("content",m)
}}var h={src:m,href:k,type:"image"};
n.addMediaItem(h)
}var o=[{provider:q.options.shareType,tooltip:"Share"}];
var g={userAction:n,enabledProviders:"*,hyves,digg,aol",containerID:"shareButton",scope:"both",privacy:"public",iconsOnly:"false",useEmailCaptcha:"true",shareButtons:o,noButtonBorders:"true",showTooltips:"true"};
gigya.socialize.showShareBarUI(g)
};
b.prototype.onSendDoneHandler=function(h){var g=this;
gigya.socialize.getUserInfo({callback:g.userInfoCallback,signIDs:true})
};
b.prototype.userInfoCallback=function(g){};
b.prototype.onErrorDoneHandler=function(h){var g=this
};
b.prototype.initClickAction=function(h){var g=this;
h.on("click",function(k){g.showShareUI()
})
};
e.fn[d]=function(g){return this.each(function(){if(!e.data(this,"plugin_"+d)){var h=new b(this,g);
e.fn[d].refArray.push(h);
e.data(this,"plugin_"+d,h)
}})
}
})(jQuery,window,document);
...
(function(e,c,a,g){var d="searchBox",f={queryParameter:null,autoComplete:false,acFooterText:null,acCharacterLimit:null,acEnablePreloader:true,acServiceUrl:null};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.$key=e(this.element).attr("id");
this.$navParent=this.$element.closest("#main-header"),this.options=e.extend({},f,h);
this._defaults=f;
this._name=d;
this.init()
}b.prototype.init=function(){if(this.$navParent.length){this.setupExpandableState()
}this.setupFormSubmission();
if(this.options.autoComplete&&this.options.acServiceUrl){this.setupAutoComplete()
}};
b.prototype.setupExpandableState=function(){var k=this.$element.find('input[type="search"]'),m=this.$element.find('input[type="submit"]'),l=this.$navParent,h=l.find(".search-toggle");
k.focusin(function(){l.removeClass("search-minimised").addClass("search-expanded")
}).blur(function(){if(l.length&&h.is(":hidden")&&!l.hasClass("autocomplete-open")){l.removeClass("search-expanded").addClass("search-minimised")
}})
};
b.prototype.setupFormSubmission=function(){var l=this.$element.find(".form").data("href"),n=this.options.queryParameter,h=this.$element.find('input[type="search"]'),m=this.$element.find('input[type="submit"]');
h.on("keypress",function(p){var o=(p.keyCode?p.keyCode:p.which);
if(o==13){m.trigger("click")
}});
m.on("click",function(q){q.preventDefault();
var p=h.val(),o=k(l,n,p);
PDC.SC.click("gsasearch","search-banner","button",{events:["event17"],vars:{eVar17:p,prop17:p}});
if(p!=""){c.location.href=o
}});
var k=function(o,q,p){if(o.indexOf("?")>0){return o+"&"+q+"="+p
}else{return o+"?"+q+"="+p
}}
};
b.prototype.setupAutoComplete=function(){if(typeof YAHOO.widget.AutoComplete=="undefined"){return false
}var m=this.$element,u=this.$element.find(".search-autocomplete")[0],v=this.$navParent,p=this.$element.find('input[type="search"]')[0],l=this.options.acFooterText,n=this.options.acCharacterLimit,t=this.$element.find('input[type="submit"]');
...
(function(e,c,a,g){var d="personalizedSection",f={mdmServiceUrl:"//secure.us.playstation.com/service/mdm/getconsumerassets"};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.options=e.extend({},f,h);
this._defaults=f;
...
if((l==="notuser"&&PDC.Profile.isLoggedIn()===false)||(l==="userloggedin"&&PDC.Profile.isLoggedIn()===true)||(l==="notsubscriber"&&PDC.Profile.isLoggedIn()===true&&PDC.Profile.getPsPlus()==="false")||(l==="subscriber"&&PDC.Profile.isLoggedIn()===true&&PDC.Profile.getPsPlus()==="true")){h.$element.removeClass("hide")
...
}if((l==="ps4owner"&&PDC.Profile.isLoggedIn()===true&&m===true)||(l==="notps4owner"&&PDC.Profile.isLoggedIn()===true&&m===false)||(l==="notps4plus"&&PDC.Profile.isLoggedIn()===true&&m===false&&PDC.Profile.getPsPlus()==="false")||(l==="ps4plus"&&PDC.Profile.isLoggedIn()===true&&m===true&&PDC.Profile.getPsPlus()==="true")){h.$element.removeClass("hide")
}})
}};
PDC.MDM=PDC.MDM||{};
...
(function(e,c,a,g){var d="leadProductInfo-followbutton",f={followGameLink:"//stagea.us.playstation.com/playstation/psn/game/follow",unFollowGameLink:"//stagea.us.playstation.com/playstation/psn/game/unfollow",checkFollowGameLink:"//stagea.us.playstation.complaystation/playstation/psn/game/checkfollow"};
function b(k,h){this.element=k;
this.$element=e(this.element);
...
if(typeof s!=="undefined"){PDC.SC.smcid_creator(k.$carousalBlock)
...
b.prototype.initClickAction=function(k,h,l){k.on("click",function(m){if(typeof s!=="undefined"){PDC.SC.open(this,e(this).data("href"),true,function(){return PDC.SC.click("kamajiMediaCarousal"+l,this,h,null)
});
return false
...
var t=(aK.hasClass("slide"))?"image":((aK.parent().hasClass("info-video")||aK.parent().hasClass("videoPlayerContainer"))?"video":"button");
if(t==="video"){return
...
}e(".image-wrap",this.$element).on("click",function(u){var t=e(this).data("href");
var o=e(this).data("href-external")=="1"?true:false;
var v=e(u.target).closest("a, .image-wrap");
if(v.hasClass("image-wrap")&&typeof t!=="undefined"&&t!==null&&t!==""){if(typeof s!=="undefined"){return PDC.SC.click("herobanner",e(this).data("image-title"),"image",{open:t,newWindow:o})
}else{if(o){c.open(t);
return false
...
b.prototype.initClickAction=function(k,h){k.on("click",function(l){if(typeof s!=="undefined"){PDC.SC.open(this,e(this).data("href"),true,function(){return PDC.SC.click("gearsproducts",this,h,null)
});
return false
}else{c.open(e(this).data("href"))
}})
...
h.$moreLink.on("click",function(){if(typeof s!=="undefined"){PDC.SC.click("gearsproducts",this,"text",null)
}h.showMore()
})
};
...
n<l.length;
n++){var r=l[n];
var q=new PDC.GameTileTemplate();
for(var m in r){q[m]=r[m]
}this.appendTile(q)
}var p=c(this).find(".priceSpider");
c.each(p,function(t,u){var w=c(u).attr("id");
var v=c(u).data("text");
var o='<script>PriceSpider.insertWidget({"sku":"'+w+'","text":"'+v+'"})<\/script>';
g.getElementById(w).innerHTML=o;
PriceSpider.rebind()
})
}};
k.prototype.appendTile=function(n){var l=this.getNextIndex();
var m=this.options.layout;
if(l&&m&&typeof l==="string"&&typeof m==="string"){if(d(m)){if(h.any()){n.addPreset(this.options.smallMobileImagePreset)
}else{n.addPreset(this.options.smallImagePreset)
}}else{if(l=="tile-1"){if(h.any()){n.addPreset(this.options.largeMobileImagePreset)
}else{n.addPreset(this.options.largeImagePreset)
}}else{if(h.any()){n.addPreset(this.options.smallMobileImagePreset)
}else{n.addPreset(this.options.smallImagePreset)
}}}n.addAgeRatingPreset(this.options.ageRatingImagePreset)
}if(d(m)){this.$ul.addGameTile(n,"no-large "+l)
}else{this.$ul.addGameTile(n,m+" "+l)
}var o=this.$ul.find("li."+l);
if(typeof s!=="undefined"){PDC.SC.smcid_creator(o)
...
}var n=this.$resultsContainer.find(".priceSpider");
d.each(n,function(r,t){var v=d(t).attr("id");
var u=d(t).data("text");
var o='<script>PriceSpider.insertWidget({"sku":"'+v+'","text":"'+u+'"})<\/script>';
...
if(typeof s!=="undefined"){if(p=="rating"){PDC.SC.click("gamefinder",this,"image",{link_section_name:p})
}else{PDC.SC.click("gamefinder",r.attr("data-filter-value"),"text",{link_section_name:p})
}}}});
...
h.addonscarousal();
if(typeof s!=="undefined"){PDC.SC.smcid_creator(h.$carousalBlock)
...
b.prototype.initClickAction=function(l,k,h){l.on("click",function(m){if(typeof s!=="undefined"){PDC.SC.open(this,e(this).data("href"),true,function(){return PDC.SC.click(h,this,k,null)
});
return false
}else{c.open(e(this).data("href"))
}})
};
b.prototype.initSectionClickAction=function(l,k,h){l.on("click",function(m){if(typeof s!=="undefined"){PDC.SC.open(this,e(this).attr("href"),false,function(){return PDC.SC.click(h,this,k,null)
});
return false
}else{c.location.href=e(this).attr("href")
}})
};
b.prototype.initClickHandler=function(){var h=this;
h.$moreLink.on("click",function(){if(typeof s!=="undefined"){PDC.SC.click("gamesaddons",this,"text",null)
}h.showMore()
})
};
b.prototype.showMore=function(){var h=this;
...
(function(e,c,a,g){var d="campaignContainer",f={themes:"PSPlusBanner",noOfOffers:"1",cacheHours:24};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.$elementParent=this.$element.parent();
this.$key=e(this.element).attr("id");
...
var m=u.options;
var n=m.defaultId;
u.campaignPage=u.$element.find(".campaignpage").data("href");
u.loginUserId=PDC.Profile.getOnlineID();
if(u.loginUserId!=null){if(!e.cookie("ac_"+m.themes+"_"+u.loginUserId)){u.getContent()
}else{if(m.themes!=="HomeSpotlight"){u.getDefaultPageContent(e.cookie("ac_"+m.themes+"_"+u.loginUserId));
if(e.cookie("ac_"+m.themes+"_"+u.loginUserId+"_cid")){n=e.cookie("ac_"+m.themes+"_"+u.loginUserId+"_cid")
}u.$element.addClass("x-sc");
u.$element.data("sc",JSON.stringify({cid:true,material_id:n,callback:function(v){if(v.vars.prop41){delete v.vars.prop41;
delete v.vars.eVar41
}}}));
u.bannerImpressions("ac"+m.themes+"logged-in")
}else{var h=e.cookie("ac_"+m.themes+"_"+u.loginUserId);
var q=h.split(",");
var t=u.$elementParent.parent();
var l=t.find(".three-column-content > .left-column > .content-wrapper > .parsys");
var r=t.find(".three-column-content > .middle-column > .content-wrapper > .parsys");
var o=t.find(".three-column-content > .right-column > .content-wrapper > .parsys");
l.load(q[0]+" .content_par");
r.load(q[2]+" .content_par");
o.load(q[1]+" .content_par");
var p=n;
if(e.cookie("ac_"+m.themes+"_"+u.loginUserId+"_cid")){p=e.cookie("ac_"+m.themes+"_"+u.loginUserId+"_cid")
}var k=p.split(",");
l.addClass("x-sc");
r.addClass("x-sc");
o.addClass("x-sc");
if(typeof k[0]!=="undefined"){l.data("sc",JSON.stringify({cid:true,material_id:k[0],callback:function(v){if(v.vars.prop41){delete v.vars.prop41;
delete v.vars.eVar41
}}}))
}if(typeof k[1]!=="undefined"){r.data("sc",JSON.stringify({cid:true,material_id:k[1],callback:function(v){if(v.vars.prop41){delete v.vars.prop41;
delete v.vars.eVar41
}}}))
}if(typeof k[2]!=="undefined"){o.data("sc",JSON.stringify({cid:true,material_id:k[2],callback:function(v){if(v.vars.prop41){delete v.vars.prop41;
delete v.vars.eVar41
}}}))
}}}}else{if(m.themes!=="HomeSpotlight"){u.getDefaultPageContent(u.campaignPage);
u.bannerImpressions("ac-banner-non-logged-in")
}}};
b.prototype.getContent=function(){var h=this;
var k=h.options;
var l=h.$element.data("href");
e.ajax({url:l+".campaign."+h.loginUserId+"."+k.themes+"."+k.noOfOffers+".json",type:"get",cache:false,dataType:"json"}).done(function(q){if(typeof(q)!="undefined"){var m="";
var u="";
if(q.results.length==1){m=q.results[0].path;
u=q.results[0].cid
}else{e.each(q.results,function(x,y){if(x==0){m=this.path;
u=this.cid
}else{m=m+","+this.path;
u=u+","+this.cid
}})
}if(m==""){m=h.campaignPage;
u=k.defaultId
}var o=new Date();
o.setTime(o.getTime()+(k.cacheHours*60*60*1000));
e.cookie("ac_"+k.themes+"_"+h.loginUserId,m,{expires:o,path:"/",domain:".playstation.com"});
if(u!==""){e.cookie("ac_"+k.themes+"_"+h.loginUserId+"_cid",u,{expires:o,path:"/",domain:".playstation.com"})
}if(k.themes==="HomeSpotlight"){var t=m.split(",");
var w=h.$elementParent.parent();
var p=w.find(".three-column-content > .left-column > .content-wrapper > .parsys");
var v=w.find(".three-column-content > .middle-column > .content-wrapper > .parsys");
var r=w.find(".three-column-content > .right-column > .content-wrapper > .parsys");
p.load(t[0]+" .content_par");
v.load(t[2]+" .content_par");
r.load(t[1]+" .content_par");
var n=u.split(",");
p.addClass("x-sc");
v.addClass("x-sc");
r.addClass("x-sc");
if(typeof n[0]!=="undefined"){p.data("sc",JSON.stringify({cid:true,material_id:n[0],callback:function(x){if(x.vars.prop41){delete x.vars.prop41;
delete x.vars.eVar41
}}}))
}if(typeof n[1]!=="undefined"){v.data("sc",JSON.stringify({cid:true,material_id:n[1],callback:function(x){if(x.vars.prop41){delete x.vars.prop41;
delete x.vars.eVar41
}}}))
}if(typeof n[2]!=="undefined"){r.data("sc",JSON.stringify({cid:true,material_id:n[2],callback:function(x){if(x.vars.prop41){delete x.vars.prop41;
delete x.vars.eVar41
}}}))
}}else{h.$element.load(m+" .content_par");
h.$element.addClass("x-sc");
h.$element.data("sc",JSON.stringify({cid:true,material_id:u,callback:function(x){if(x.vars.prop41){delete x.vars.prop41;
delete x.vars.eVar41
}}}));
h.bannerImpressions("ac"+k.themes+"logged-in")
}}else{if(k.themes!=="HomeSpotlight"){var m=h.campaignPage;
h.$element.load(m+" .content_par");
h.bannerImpressions("ac"+k.themes+"logged-in")
}}})
};
b.prototype.getDefaultPageContent=function(k){var h=this;
h.$element.load(k+" .content_par")
};
b.prototype.bannerImpressions=function(m){var h=this;
var k=h.options;
if(typeof s!=="undefined"){var l={type:"impression"};
PDC.SC.find_sc_provider(h.$element,l);
l.events=["event5"];
l.vars={list1:m};
PDC.SC.click("adobecampaigncontainer",k.themes,"image",l)
}else{var l={type:"impression"};
l.events=["event5"];
l.vars={list1:m};
h.$element.attr("data-impression",'{"componentCtx" : "adobecampaigncontainer", "entity" : "image", "options" :'+JSON.stringify(l)+"}")
}};
e.fn[d]=function(h){return this.each(function(){if(!e.data(this,"plugin_"+d)){e.data(this,"plugin_"+d,new b(this,h))
}})
}
...
(function(e,c,a,g){var d="userNotificationWall",f={wallFeedDataUrl:"//io.playstation.com/playstation/psn/messagecenter/message/news",publicWallFeedDataUrl:"//io.playstation.com/playstation/psn/messagecenter/message/news/off",activityDataUrl:"//io.playstation.com/playstation/psn/messagecenter/message/useractivity",unreadMessageNotificationDataUrl:"//io.playstation.com/playstation/psn/messagecenter/message/notifications",templateId:""};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.options=e.extend({},f,h);
this._defaults=f;
...
this.isLoggedIn=e.cookie("userinfo")?true:false;
this.feedDataUrl=this.isLoggedIn?this.options.wallFeedDataUrl:this.options.publicWallFeedDataUrl;
this.init()
}b.prototype.init=function(){var h=this;
h.$firstColumn.packery({columnWidth:300,itemSelector:".grid",gutter:15});
h.loadData();
h.switchView();
...
b.prototype.switchView=function(){var h=this;
h.$switch=h.$element.find(".switchContainer div");
h.$switch.bind("click",function(){if(e(this).hasClass("notSelected")){e(this).siblings(".selected").removeClass("selected blueWhite").addClass("notSelected greyBlack");
e(this).removeClass("notSelected greyBlack").addClass("selected blueWhite")
}})
};
b.prototype.loadData=function(){var h=this;
h.$switchDiv=h.$element.find(".switchContainer");
var k={pageNum:h.pageNum,type:4};
if(h.isLoggedIn){k.userInfoCookie=e.cookie("userinfo");
h.$switchDiv.removeClass("hide")
}e.ajax({type:"get",async:true,crossDomain:true,data:k,url:h.feedDataUrl,dataType:"json"}).done(function(l){h.totalPages=Math.ceil(l.totalCampaigns/h.tilesPerPage);
if(l.list.length>=1){h.dataNewsList=h.dataNewsList.concat(l.list.slice());
h.$errorMsg.addClass("hide");
e.each(l.list,function(m,o){h.$firstColumn.append(tmpl(h.options.templateId,o));
var n=h.$firstColumn.children().last();
h.$firstColumn.packery("appended",n);
h.initClickHandlers(n)
});
...
var k=l.children("a");
k.on("click",function(u){var v=e(this).data("campaign");
var n=h.dataNewsList.length;
for(var q=0;
q<n;
q++){var p=h.dataNewsList[q].campaignId;
if(h.dataNewsList[q].content!=g){e(this).removeAttr("href");
if(p==v){var r=h.dataNewsList[q].content;
var w=h.dataNewsList[q].author;
var m=h.dataNewsList[q].name;
var y=h.dataNewsList[q].url;
var o=h.dataNewsList[q].imgSrc;
var x="<style>.modalWrap *{font-family:'SST W01 Roman','SST W02 Roman','SST W10 Roman','SST W15 Roman',Arial,Helvetica,sans-serif !important} .modalWrap h1{font-family:'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif !important}.modalWrap {margin:2px 2px 0px}.modalWrap #modalTopImage{width:100%; height: auto !important; max-height:500px;}.modalWrap .modalContentWrapper{width:92%; margin:2% auto}.modalWrap h1{font-size: 24px; font-family: 'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif;}.modalWrap p{font-family:'SST W01 Roman','SST W02 Roman','SST W10 Roman','SST W15 Roman',Arial,Helvetica,sans-serif;font-size: 16px} .modalWrap p img{margin:5px; height: auto !important} .modalWrap .modalButtondiv{	margin: 5% auto;max-width: 500px;text-align: center;}.modalWrap .modalButtondiv #modalButton{ background-color: #0068bf;background-image: linear-gradient(top,#008ae3,#0070c7);	background-image: -moz-linear-gradient(top,#008ae3,#0070c7);background-image: -ms-linear-gradient(top,#008ae3,#0070c7);	background-image: -o-linear-gradient(top,#008ae3,#0070c7);background-image: -webkit-linear-gradient(top,#008ae3,#0070c7);color: #ffffff;font: 18px/41px 'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif !important; padding: 12px 15px;}.modalWrap .modalButtondiv #modalButton span{font-family: 'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif !important} .modalWrap img{max-width: 100%} .modalWrap iframe{margin: 5px; width:98%;} @media screen and (max-width: 768px){.modalWrap p{font-size: 14px !important; font-family: 'SST W01 Roman','SST W02 Roman','SST W10 Roman','SST W15 Roman',Arial,Helvetica,sans-serif;}.modalWrap h1 {font-size: 24px!important; font-family: 'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif}.modalWrap .modalButtondiv span{font-size: 14px ;font-family: 'SST W01 Roman','SST W02 Roman','SST W10 Roman','SST W15 Roman',Arial,Helvetica,sans-serif !important}}</style>";
var t="<div class='modalWrap'><img id='modalTopImage' src="+o+"></img><br><div class='modalContentWrapper'><h1>"+m+"</h1><p style='font-size:16px; line-height:26px;'>Posted by "+w+"</p><br><hr style='color:#1f1f1f;'></hr><br><p id='modalContent'>"+r+"</p><div class='modalButtondiv'><a id='modalButton'><span display='block'>VIEW ORIGINAL ARTICLE</span></a></div></div></div>";
if(e(c).width()<=768){topDistance="20px"
}else{topDistance="150px"
}e.colorbox({scrolling:false,inline:true,width:"60%",html:true,top:topDistance,onComplete:function(){e("#cboxContent").css("height","auto");
e("#cboxLoadedContent").append(x+t).css("height","auto");
var A=e(".modalWrap #modalTopImage").width();
e(".modalWrap #modalTopImage").height(A);
var z=e(".modalWrap iframe").width();
e(".modalWrap iframe").height(z*0.56);
e(".modalWrap .modalButtondiv").click(function(){if(typeof s!=="undefined"){return PDC.SC.click("button","view original article","psnews",{vars:{prop56:"playstation.blog",eVar56:"playstation.blog"},events:["event54"],open:y,newWindow:true})
}else{c.open(y)
}})
}});
if(typeof s!=="undefined"){PDC.SC.click("psnews",this,"image",{linktitle_override:"_subject",_subject:e(this).find(".subject").text(),callback:function(z){delete z.vars.prop56;
delete z.vars.eVar56;
z.events.pop("event54")
}})
}}}else{if(h.dataNewsList[q].content==g&&p==v){if(typeof s!=="undefined"&&e(this).data("href")!=="javascript:void(0);"){PDC.SC.click("psnews",this,"image",{linktitle_override:"_subject",_subject:e(this).find(".subject").text(),open:e(this).data("href"),newWindow:true})
}else{c.open(e(this).data("href"))
}}}}return false
});
l.find(".post-like-box").on("click",function(q){q.stopPropagation();
if(typeof s!=="undefined"){var p=e(this).hasClass("no");
var r="like-intent";
if(h.isLoggedIn){r=p?"like":"unlike"
}var o=e(this).closest(".grid");
var m=o.find(".subject").text();
var n={linktitle_override:"_subject",_subject:m,link_section_name:r,vars:{prop46:"myplaystation",eVar46:"myplaystation"}};
if(p){n.events=["event46"]
}PDC.SC.click("psnews",this,"image",n)
}if(h.isLoggedIn){h.likedPost(e(this))
}else{PDC.MyPlaystationProfile.redirectToLogin()
}})
};
b.prototype.likedPost=function(o){var k=this;
k.$loadingOverlay.removeClass("hide");
var n=o.hasClass("no");
var m=n?"Like":"Unlike";
var h=o.parents(".post-box");
var l=h.find(".likeCountText").text();
e.ajax({type:"POST",url:k.options.activityDataUrl,data:{id:o.data("campaign"),action:m,type:"resource",userInfoCookie:e.cookie("userinfo")},dataType:"json"}).done(function(q){k.$loadingOverlay.addClass("hide");
if(typeof q.status!=="undefined"&&q.status==="success"){var p=q.likeCount;
if(m==="Unlike"&&parseInt(l)>0){p=parseInt(l)-1
}else{if(typeof p==="undefined"||parseInt(p)<1){p="0"
}}e("._item."+o.data("campaign")).each(function(){e(this).find(".like-count .likeCountText").html(p);
if(n){e(this).find(".post-box .post-like-box").addClass("yes");
e(this).find(".post-box .post-like-box").removeClass("no")
}else{e(this).find(".post-box .post-like-box").addClass("no");
e(this).find(".post-box .post-like-box").removeClass("yes")
}})
}})
};
e.fn[d]=function(h){return this.each(function(){if(!e.data(this,"plugin_"+d)){e.data(this,"plugin_"+d,new b(this,h))
...
h.$element.find(".sectionContainer").removeClass("hide")
...
var k=h.options;
var m="";
if(k.pageType==="public"){m="&compare="+n
...
(function(e,c,a,g){var d="publicRecentlyPlayedTrophyGrid",f={};
function b(l,k){var h=this;
h.element=l;
h.$element=e(h.element);
h.$elementParent=h.$element.parent();
...
if(c.navigator.userAgent.toString().indexOf("PLAYSTATION 3")>-1){h.$element.find(".button-row").addClass("hide")
}else{h.$element.find(".tile-link").on("click",function(){if(typeof s!=="undefined"){var n=e(this).find("h2.title").text();
var l=e(this).find(".btn").text();
PDC.SC.click("recentlyplayedgames",l,"image",{link_section_name:n})
}var k=e(this).data("gameid");
var m=e(this).data("handle");
e.cookie("compareTrophiesVars","gid="+k+"&compare="+m,{domain:".playstation.com",path:"/"});
c.location=e(this).data("href")
})
}};
e.fn[d]=function(h){return this.each(function(){if(!e.data(this,"plugin_"+d)){var k=new b(this,h);
e.fn[d].refArray.push(k);
...
e(q).each(function(J,L){var G=e("<li>").attr({"class":"tile","data-isonline":L.isOnline,"data-handle":L.handle}).appendTo(k);
...
if("isFavourite" in L){if(parseInt(L.isFavourite)===1){z.addClass("favourite");
g=Adobe.I18n.get("Favorite")
}else{g=Adobe.I18n.get("Favorite")
}}else{z.addClass("favourite");
g="favourite"
}var P=e("<div>").attr({"class":"friend clearfix"}).appendTo(z);
if(L.isPlusUser==="1"||L.isPlusUser==="true"){e("<div>").attr({"class":"ps-plus"}).html("&nbsp;").appendTo(P)
}e("<div>").attr({"class":"avatar","data-handle":L.handle}).html('<img width="100" height="100" alt="'+L.handle+'" title="'+L.handle+'" src="'+L.avatarUrl+'"/>').css({cursor:"pointer"}).appendTo(P).on("click",function(){if(c.navigator.userAgent.toString().indexOf("PLAYSTATION 3")>-1){}else{c.location=r.publicProfileDomain+e(this).data().handle
}});
e("<div>").attr({"class":"level"}).html("<p>"+Adobe.I18n.get("Level")+"</p><span>"+L.curLevel+"</span>").appendTo(P);
var A=e("<div>").attr({"class":"compete-tile-details"}).appendTo(z);
var H=r.groupType==="friends"?L.friendId:L.platform;
var y=e("<div>").attr({"class":"fixed-container"}).appendTo(A);
e("<h2>").attr({"class":"clearfix title","data-handle":L.handle}).html(L.handle).appendTo(y).css({cursor:"pointer"}).on("click",function(){if(c.navigator.userAgent.toString().indexOf("PLAYSTATION 3")>-1){}else{c.location=r.publicProfileDomain+e(this).data().handle
}});
var D=L.trophies;
...
var I=e("<div>").attr({"class":"button-row"}).appendTo(z);
var M="eventVal=event66,secName=button:compare:usr-"+L.handle.replace(/\s/g,"-").toLowerCase();
var K=e("<div>").attr({href:"javascript:void(0);","class":"comparebutton","data-handle":L.handle,"data-game-id":v.trophiesData.gameId}).appendTo(I);
if(r.useUrlVars){K.attr({href:r.compareLink+"#compare="+L.handle})
}else{K.on("click",t)
}if(c.navigator.userAgent.toString().indexOf("PLAYSTATION 3")>-1){}else{var w=e("<a>").attr({"class":"link-btn blue-btn small-btn"}).appendTo(K);
var N=e("<div>").attr({"class":"btn"}).html(Adobe.I18n.get("Compare")).appendTo(w);
var E="eventVal=event66,secName=button:favourite:usr-"+L.handle.replace(/\s/g,"-").toLowerCase();
var O=e("<a>").attr({href:"javascript:void(0);","data-handle":L.handle,"class":"starButton",title:g}).appendTo(I).on("click",function(){v.setFavourite(e(this))
});
var C=e("<div>").attr({"class":"link-btn blue-btn starbtn"}).appendTo(O);
var N=e("<div>").attr({"class":"btn"}).html("").appendTo(C)
}});
m=k.find(".tile").length;
...
(function(f,d,a,h){var e="publicProfileHeroBanner",g={respondWidth:970};
function c(m,l){var k=this;
k.element=m;
k.$element=f(k.element);
k.options=f.extend({},g,l);
k._defaults=g;
k._name=e;
k.init()
}c.prototype.init=function(){var k=this;
f(d).load(function(){k.isWindowLoaded=true
});
k.initUserInfo()
};
c.prototype.initUserInfo=function(){var o=this;
f(d).load(function(){o.growLevel()
});
if(o.isWindowLoaded){o.growLevel()
}if("matchMedia" in d){function m(p){o.growLevel()
}var l=d.matchMedia("(max-width: "+o.options.respondWidth+"px)");
m(l);
l.addListener(m)
}else{var k;
o.growLevel();
f(d).resize(function(){if(k){clearTimeout(k)
}k=setTimeout(n,500)
});
function n(){o.growLevel()
}}};
c.prototype.growLevel=function(){var l=this;
var o=l.$element.find(".level-guage span");
var k=l.$element.find(".level-guage").width();
var m=k*(o.text()/100);
var n=l.$element.find(".level-guage").find(".highlight");
n.stop().animate({width:m},{duration:2000,easing:"swing"})
};
(function(n,k,o){var m,l=n.getElementsByTagName(k)[0];
if(n.getElementById(o)){return
}m=n.createElement(k);
m.id=o;
m.src="//connect.facebook.net/en_US/all.js#xfbml=1&appId=5437122869";
l.parentNode.insertBefore(m,l)
}(a,"script","facebook-jssdk"));
d.fbAsyncInit=function(){FB.Event.subscribe("edge.create",function(k){var l=f("#fakeLink")[0];
b.bind(l)("facebook",true)
})
};
d.twttr=(function(o,l,p){var k,n,m=o.getElementsByTagName(l)[0];
if(o.getElementById(p)){return
}n=o.createElement(l);
n.id=p;
n.src="https://platform.twitter.com/widgets.js";
m.parentNode.insertBefore(n,m);
return d.twttr||(k={_e:[],ready:function(q){k._e.push(q)
}})
}(a,"script","twitter-wjs"));
twttr.ready(function(k){k.events.bind("tweet",function l(){var m=f("#fakeLink")[0];
b.bind(m)("twitter",true)
})
});
function b(k,m){if(!m){return
}var n=k;
var l={vars:{eVar46:n,prop46:n}};
if(n=="facebook"||n=="google"){l.events=["event46"]
}else{if(n=="twitter"){l.events=["event48"]
}}PDC.SC.click("publicprofileherobanner",n,"image",l)
}f.fn[e]=function(k){return this.each(function(){if(!f.data(this,"plugin_"+e)){f.data(this,"plugin_"+e,new c(this,k))
}})
}
})(jQuery,window,document);
...
(function(e,c,a,g){var d="myWall",f={wallFeedDataUrl:"//io.playstation.com/playstation/psn/messagecenter/message/wallfeeds",wallFeedDetailsDataUrl:"//io.playstation.com/playstation/psn/messagecenter/message/wallpostdetails",activityDataUrl:"//io.playstation.com/playstation/psn/messagecenter/message/useractivity",avatarDataUrl:"//io.playstation.com/playstation/psn/profile/avatar/",unreadMessageNotificationDataUrl:"//io.playstation.com/playstation/psn/messagecenter/message/notifications",visitUrl:"//io.playstation.com/playstation/psn/messagecenter/message/visit",templateId:"",profileLinkURL:"https://my.playstation.com/"};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.dataWallList=[];
this.options=e.extend({},f,h);
...
h.$firstColumn.packery({columnWidth:300,itemSelector:".grid",gutter:15});
PDC.MyPlaystationProfile.getSignedUserData(function(k){if(k.handle){h.handle=k.handle;
e.ajax({type:"POST",async:true,crossDomain:true,data:{userInfoCookie:e.cookie("userinfo")},url:h.options.visitUrl,dataType:"json"}).always(function(l){if(e(".avatar a.nav-content span .notification-wrapper").length>0){PDC.UserNotificatio.getNotifications(h.options.unreadMessageNotificationDataUrl)
}});
h.loadData();
h.switchView();
...
b.prototype.dateformatter=function(){var h=this;
h.$DOP=h.$columnContainer.find(".dateOfPost");
var k=["January","February","March","April","May","June","July","August","September","October","November","December"];
h.$DOP.each(function(l){var o=e(this).text();
if(o==""){o=e(this).data("datetime");
function n(q){if(q>3&&q<21){return"th"
}switch(q%10){case 1:return"st";
case 2:return"nd";
case 3:return"rd";
default:return"th"
}}o=o.split("T")[0];
...
var p=(o[1]).slice(-2);
p=parseInt(p)-1;
var m=k[p]+" "+parseInt(o[2])+n(o[2])+" "+o[0];
e(this).text(m);
e(this).removeAttr("data-datetime")
}})
};
b.prototype.loadData=function(){var h=this;
...
if(p.Status==="No Record Found"||e.isEmptyObject(p)||typeof p.list==="undefined"||p.list===null||p.list.length<1){h.$loadingOverlay.addClass("hide");
h.$moreLink.addClass("hide");
h.$errorMsg.removeClass("hide")
}else{if(p.list.length>=1){h.dataWallList=h.dataWallList.concat(p.list.slice());
h.totalPages=Math.ceil(p.totalCampaigns/h.tilesPerPage);
e.each(p.list,function(q,t){h.$firstColumn.append(tmpl(h.options.templateId,t));
var r=h.$firstColumn.children().last();
h.$firstColumn.packery("appended",r);
h.initClickHandlers(r)
});
if(h.pageNum===h.totalPages){h.$moreLink.addClass("hide")
}h.$loadingOverlay.addClass("hide");
...
b.prototype.lazyLoadAvatars=function(){var k=this;
k.tile=k.$columnContainer.find(".grid > a");
var l=0;
var h=[];
k.$columnContainer.find("img.notDone").each(function(m){var n=e(this);
var o=n.data("img-src");
if(typeof o==="undefined"||o===null||o===""){o=k.handle
}n.attr("class",o);
if(!k.uniquieHandles.hasOwnProperty(o)){k.uniquieHandles[o]="undefined";
h[l]=o;
l=l+1
}else{if(k.uniquieHandles[o]!="undefined"){n.attr("src",k.uniquieHandles[o]).removeClass("notDone").addClass("done")
}}});
e.each(h,function(m,n){e.ajax({type:"GET",url:k.options.avatarDataUrl+n,dataType:"json"}).success(function(o){k.tile.find("."+n).attr("src",o.avatarUrl).removeClass("notDone").addClass("done");
k.uniquieHandles[n]=o.avatarUrl
})
})
};
b.prototype.initClickHandlers=function(m){var h=this;
var k=m.children("a");
var n=h.options.profileLinkURL;
if(!k.hasClass("trophy")){k.on("click",function(w){if(e(this).hasClass("blogstile")){var x=e(this).data("campaign");
var p=h.dataWallList.length;
for(var t=0;
t<p;
t++){var r=h.dataWallList[t].campaignId;
if(h.dataWallList[t].content!=g){if(r==x){var u=h.dataWallList[t].content;
var y=h.dataWallList[t].author;
var o=h.dataWallList[t].name;
var A=h.dataWallList[t].url;
var q=h.dataWallList[t].imgSrc;
var z="<style>.modalWrap *{font-family:'SST W01 Roman','SST W02 Roman','SST W10 Roman','SST W15 Roman',Arial,Helvetica,sans-serif !important} .modalWrap h1{font-family:'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif !important}.modalWrap {margin:2px 2px 0px}.modalWrap #modalTopImage{width:100%; height: auto !important; max-height:500px;}.modalWrap .modalContentWrapper{width:92%; margin:2% auto}.modalWrap h1{font-size: 24px; font-family: 'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif;}.modalWrap p{font-family:'SST W01 Roman','SST W02 Roman','SST W10 Roman','SST W15 Roman',Arial,Helvetica,sans-serif;font-size: 16px} .modalWrap p img{margin:5px; height: auto !important} .modalWrap .modalButtondiv{	margin: 5% auto;max-width: 500px;text-align: center;}.modalWrap .modalButtondiv #modalButton{ background-color: #0068bf;background-image: linear-gradient(top,#008ae3,#0070c7);	background-image: -moz-linear-gradient(top,#008ae3,#0070c7);background-image: -ms-linear-gradient(top,#008ae3,#0070c7);	background-image: -o-linear-gradient(top,#008ae3,#0070c7);background-image: -webkit-linear-gradient(top,#008ae3,#0070c7);color: #ffffff;font: 18px/41px 'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif !important; padding: 12px 15px;}.modalWrap .modalButtondiv #modalButton span{font-family: 'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif !important} .modalWrap img{max-width: 100%} .modalWrap iframe{margin: 5px; width:98%;} @media screen and (max-width: 768px){.modalWrap p{font-size: 14px !important; font-family: 'SST W01 Roman','SST W02 Roman','SST W10 Roman','SST W15 Roman',Arial,Helvetica,sans-serif;}.modalWrap h1 {font-size: 24px!important; font-family: 'SST W01 Bold','SST W02 Bold','SST W10 Bold','SST W15 Bold','Arial Bold',Arial,Helvetica,sans-serif}.modalWrap .modalButtondiv span{font-size: 14px ;font-family: 'SST W01 Roman','SST W02 Roman','SST W10 Roman','SST W15 Roman',Arial,Helvetica,sans-serif !important}}</style>";
var v="<div class='modalWrap'><img id='modalTopImage' src="+q+"></img><br><div class='modalContentWrapper'><h1>"+o+"</h1><p style='font-size:20px; line-height:26px;'>Posted by "+y+"</p><br><hr style='color:#1f1f1f;'></hr><br><p id='modalContent'>"+u+"</p><div class='modalButtondiv'><a id='modalButton'><span display='block'>VIEW ORIGINAL ARTICLE</span></a></div></div></div>";
if(e(c).width()<=768){topDistance="20px"
}else{topDistance="150px"
}e.colorbox({scrolling:false,inline:true,width:"60%",html:true,top:topDistance,onComplete:function(){e("#cboxContent").css("height","auto");
e("#cboxLoadedContent").append(z+v).css("height","auto");
var D=e(".modalWrap #modalTopImage").width();
e(".modalWrap #modalTopImage").height(D);
var C=e(".modalWrap iframe").width();
e(".modalWrap iframe").height(C*0.56);
e(".modalWrap .modalButtondiv").click(function(){if(typeof s!=="undefined"){return PDC.SC.click("button","view original article","mywall",{vars:{prop56:"playstation.blog",eVar56:"playstation.blog"},events:["event54"],open:A,newWindow:true})
}else{c.open(A)
}})
}});
if(typeof s!=="undefined"&&e(this).data("href")!=="javascript:void(0);"){PDC.SC.click("mywall",this,"image",{linktitle_override:"_subject",_subject:e(this).find(".subject").text(),callback:function(C){delete C.vars.prop56;
delete C.vars.eVar56;
C.events.pop("event54")
}})
}}}else{if(h.dataWallList[t].content==g&&r==x){if(typeof s!=="undefined"&&e(this).data("href")!=="javascript:void(0);"){PDC.SC.click("mywall",this,"image",{linktitle_override:"_subject",_subject:e(this).find(".subject").text(),open:e(this).data("href"),newWindow:true})
}else{c.open(e(this).data("href"))
}}}}}if(e(this).hasClass("newstile")){var B=e(this).data("href");
if(typeof B!=="undefined"&&B!==null&&B!==""&&B!=="javascript:void(0);"){if(typeof s!=="undefined"){PDC.SC.click("mywall",this,"image",{linktitle_override:"_subject",_subject:e(this).find(".subject").text(),open:B,newWindow:false})
}else{a.location.href=B
}}}return false
})
}else{if(k.hasClass("trophy")){var l=n+k.find("img").data("img-src");
k.find(".profileRedirect, strong").on("click",function(){if(typeof s!=="undefined"){PDC.SC.click("mywall","username","text",{open:l,newWindow:true})
}else{c.open(l)
}})
}}m.find(".post-like-box .social-btn-like").on("click",function(){if(typeof s!=="undefined"){var q=e(this).closest(".post-like-box").hasClass("no");
var r=q?"like":"unlike";
var p=e(this).closest(".grid");
var o=p.find(".subject").text();
PDC.SC.click("mywall",this,"image",{linktitle_override:"_subject",_subject:o,link_section_name:r})
}h.likedPost(e(this))
})
};
b.prototype.likedPost=function(p){var h=this;
h.$loadingOverlay.removeClass("hide");
var o=p.closest(".post-like-box").hasClass("no");
var m=p.closest(".grid");
var l=m.children("a").hasClass("trophy")?"trophy":"resource";
var n=o?"Like":"Unlike";
var k=m.find(".likeCountText").text();
e.ajax({type:"POST",url:h.options.activityDataUrl,data:{id:p.data("campaign"),action:n,type:l,userInfoCookie:e.cookie("userinfo")},dataType:"json"}).done(function(r){h.$loadingOverlay.addClass("hide");
if(typeof r.status!=="undefined"&&r.status==="success"){var q=r.likeCount;
if(n==="Unlike"&&parseInt(k)>0){q=parseInt(k)-1
}else{if(typeof q==="undefined"||parseInt(q)<1){q="0"
}}e("._item."+p.data("campaign")).each(function(){e("._item."+p.data("campaign")+" .post-box .count-box .like-count .likeCountText").html(q);
if(o){e(this).children(".post-box").children(".social-icon-box").children(".post-like-box").addClass("yes");
...
(function(e,c,a,g){var d="followedGames",f={unFollowGameLink:"//stagea.us.playstation.com/playstation/psn/game/unfollow",followedGameLink:"//stagea.us.playstation.com/playstation/psn/game/follows",tilesPerPage:4,curPage:0,userFollowedGamesData:null};
function b(l,k){var h=this;
h.element=l;
h.$element=e(h.element);
h.$elementParent=h.$element.parent();
...
h._defaults=f;
h._name=d;
h.$headingWrapper=h.$element.prev(".headingWrapper");
h.$messageWrapper=h.$element.siblings(".followed-games-tile-grid-messages-wrapper");
h.$unfollowedGames=[];
h.increment=0;
PDC.MyPlaystationProfile.getSignedUserData(function(m){if(m.handle){h.init()
}else{PDC.MyPlaystationProfile.redirectToLogin()
...
b.prototype.fetchData=function(){var h=this;
var k=h.options;
h.$ajaxPreloader=e("<div>").attr({"class":"ajax-loader"}).appendTo(".followedGamesTileGrid");
e(".followedGamesTileGrid").css({height:"290px",position:"relative"});
e.ajax({url:k.followedGameLink,dataType:"json",data:{userInfoCookie:e.cookie("userinfo")},cache:false}).done(function(l){h.followedGamesData=k.userFollowedGamesData=l;
h.buildDom()
}).fail(function(n,m,l){h.$element.prev(".headingWrapper").addClass("hide");
h.$element.addClass("hide");
h.followedGamesData=null;
h.buildDom()
})
};
b.prototype.unfollowGame=function(k){var h=this;
var l=h.options;
h.curList=h.followedGamesData.list.slice(0);
var m=h.curList.length;
if(typeof s!=="undefined"){PDC.SC.click(l.componentCtx,"unfollow","button",null)
}e.ajax({url:l.unFollowGameLink,type:"post",dataType:"json",cache:false,data:{userInfoCookie:e.cookie("userinfo"),sceaId:k,operation:"unfollow"}}).done(function(o){if(o.status==="success"){var n=[];
n=k;
h.$unfollowedGames=h.$unfollowedGames.concat(n);
h.$element.find("ul.tile-grid-gamesFollowed").html("");
h.buildTileList(true,k)
}})
};
b.prototype.buildDom=function(){var h=this;
var k=h.options;
var m=h.followedGamesData;
var l=0;
k.curPage=0;
h.$element.find("ul.tile-grid-gamesFollowed").html("");
h.$messageWrapper.find(".no-results-main").addClass("hide");
if(m&&m.list){l=m.list.length;
h.totalPage=Math.ceil(l/k.tilesPerPage);
if(h.$ajaxPreloader){h.$ajaxPreloader.remove();
e(".followedGamesTileGrid").css({height:"",position:""})
}if(l>0){h.$headingWrapper.removeClass("hide");
h.$element.find(".sectionContainer").removeClass("hide");
h.$element.addClass("moduleComponent clearfix followedGamesTileGrid");
h.$element.removeClass("ajax-loader");
if(h.totalPage>1){h.$element.find("a.more-link").removeClass("hide").on("click",function(){if(typeof s!=="undefined"){PDC.SC.click("page-section",this,"text",null)
}k.curPage++;
if(k.curPage>=h.totalPage-1){e(this).addClass("hide");
k.curPage=h.totalPage-1
}h.buildTileList(false)
})
}}else{h.showErrorMsg(true)
}}else{if(!m.list){h.showErrorMsg(true)
}else{h.showErrorMsg(false)
}}if(l>0){h.buildTileList(false)
}};
b.prototype.showErrorMsg=function(l){var h=this;
var k=h.options;
if(k.componentCtx==="topfollowedgame"){h.$messageWrapper.addClass("hide")
}else{if(l){h.$messageWrapper.removeClass("hide").find(".no-data .no-results-main").removeClass("hide")
}else{h.$messageWrapper.removeClass("hide").find(".ajax-fail .no-results-main").removeClass("hide")
}}h.$headingWrapper.addClass("hide");
h.$element.addClass("hide");
h.$element.removeClass("moduleComponent clearfix followedGamesTileGrid").removeClass("ajax-loader");
h.$element.html("")
};
b.prototype.followedCountMsg=function(m,n){var h=this;
var k=h.options;
var l=h.$headingWrapper.find(".count-label");
h.countMsgHtml=Adobe.I18n.get("Showing {COUNT} of {TOTALCOUNT} Games");
h.countMsgHtml=h.countMsgHtml.replace("{COUNT}","<strong>"+m+"</strong>");
h.countMsgHtml=h.countMsgHtml.replace("{TOTALCOUNT}","<strong>"+n+"</strong>");
if(m>0){l.css({visibility:"visible"}).html(h.countMsgHtml)
}else{l.css({visibility:"hidden"})
}};
b.prototype.buildTileList=function(o,p){var r=this;
var n=r.options;
r.curList=r.followedGamesData.list.slice(0);
var h=r.curList.length;
var q=n.curPage*n.tilesPerPage;
var l=q+n.tilesPerPage;
l=l>=h?h:l;
if(o&&l<h){r.increment=r.increment+1;
l=l+r.increment
}var k=r.$element.find("ul.tile-grid-gamesFollowed .tile").length;
if(n.componentCtx==="myFollowedGames"){r.$element.closest("page-section").addClass("pull-up");
r.$element.prev(".headingWrapper").addClass("pull-up clearfix")
}if(k<q){q=k
}var m=r.curList.slice(q,l);
e(m).each(function(H,I){var H=0;
var B=e("<li>").attr({"class":"tile",}).appendTo(".tile-grid-gamesFollowed");
for(H==0;
H<r.$unfollowedGames.length;
H++){if(r.$unfollowedGames[H]==I.gameId){B.css("display","none")
}}var v=e("<div>").attr({"class":"compete-tile"}).appendTo(B);
var M=e("<div>").html(I.title).html();
var x=e("<a>").attr({"class":"tile-link clearfix","data-gameid":I.gameId,"data-gameLink":I.gamelink,href:I.gameLink}).appendTo(v);
var E=I.imgUrl;
if(E!=null){E=E.replace("Box_Art_Image","Icon")
}else{E="/etc/designs/pdc/clientlibs_base/images/game_placeholder.png"
}e("<div>").attr({"class":"followedGame-image"}).html('<img style="width:200px;height:229px;display:block;" src="'+E+'" alt="'+M+'" title= "'+M+'">').appendTo(x);
var z=e("<div>").attr({"class":"compete-tile-details"}).appendTo(x);
var L=I.platform?I.platform.split(","):[];
e(L).each(function(N,O){if(O.match(/ps3/i)){L[N]="PS3"
}else{if(O.match(/vita/i)){L[N]="PS VITA"
}else{if(O.match(/ps2/i)){L[N]="PS2"
}else{if(O.match(/psp/i)){L[N]="PSP"
}else{if(O.match(/ps4/i)){L[N]="PS4"
}}}}}});
I.platform=L.join(", ");
var C=I.platform;
var G=I.platform;
var u=Adobe.I18n.get("Following Game");
var J=Adobe.I18n.get("Unfollow Game");
var A=G.replace(/\s/g,"");
A=A.split(",");
var D="";
for(var H=0;
H<A.length;
H++){D+="<span class='titleLogo"+A[H]+"'>"+C+"</span>"
}var w=e("<div>").attr({"class":"fixed-container"}).html(D).appendTo(z);
e("<h2>").attr({"class":"clearfix title"}).html(M).appendTo(w);
x.on("click",function(){return PDC.SC.click("gametile",this,"image",null)
});
if(c.navigator.userAgent.toString().indexOf("PLAYSTATION 3")===-1){var F=e("<div>").attr({"class":"unfollow-button-row",}).appendTo(v);
var K=e("<img>").attr({"class":"btnimage"}).appendTo(F);
var t=e("<div>").attr({"class":"blue-btn-follow unfollowGame"}).appendTo(F);
var y=e("<span>").attr({"class":"textFollow"}).html(u).appendTo(t);
var y=e("<span>").attr({"class":"textUnfollow",style:"display: none;"}).html(J).appendTo(t);
t.bind("click",function(){var N=e(this).parent().siblings(".tile-link").data("gameid");
r.unfollowGame(N)
})
}});
k=r.$element.find(".tile-grid-gamesFollowed").find(".tile").length;
if(r.$unfollowedGames.length>0){k=k-r.$unfollowedGames.length;
h=h-r.$unfollowedGames.length
}r.followedCountMsg(k,h);
r.$element.find(".unfollow-button-row").hover(function(){e(this).find(".textUnfollow").css({display:"block"});
e(this).find(".textFollow").css({display:"none"})
},function(){e(this).find(".textUnfollow").css({display:"none"});
e(this).find(".textFollow").css({display:"block"})
});
if(e(a).width()<900){r.$element(".unfollow-button-row").off("hover")
}};
e.fn[d]=function(h){return this.each(function(){if(!e.data(this,"plugin_"+d)){var k=new b(this,h);
e.fn[d].refArray.push(k);
e.data(this,"plugin_"+d,k)
...
(function(e,c,a,g){var d="competeConnectBanner",f={level_percentage:0,respondWidth:970,userInfoDataUrl:"//io.playstation.com/userinfo.json",onlineFriendsDataUrl:"//io.playstation.com/onlinefriends.json",playstationPlusDataUrl:"//io.playstation.com/playstation/psn/plus/info",onlineFriendsData:null,userInfoData:null,pspLogoUrl:"/etc/designs/pdc/clientlibs_base/images/CM156_CompeteConnectHeroBanner/psp_logo.png",pageType:"private",sonyRewardPointLink:"//io.playstation.com/psnevent/sonyrewards",lastPlayedCall:"//io.playstation.com/playstation/psn/profile/last/played/",publicProfileDomain:"//my.playstation.com/"};
...
h.$gigyaLoginButton=h.$contentWrap.find(".gigya-addon");
var k=h.options;
...
h.buildPlusExpDom(l);
if(typeof gigya!=="undefined"&&h.$gigyaLoginButton.length){e(c).trigger("socialLoginVerification")
}};
...
b.prototype.buildPublicProfileDom=function(){var h=this;
var l=h.options;
var n=h.$element.find(".publicProfile");
if(n.length>0&&typeof l.publicProfileDomain!=="undefined"&&l.publicProfileDomain!==null&&l.publicProfileDomain!==""){var k=l.publicProfileDomain+PDC.Profile.getOnlineID();
var m=e("<a>").attr({href:k}).appendTo(n);
m.text(k);
m.on("click",function(){return PDC.SC.click("competeconnectherobanner",this,"text",null)
});
n.removeClass("hide")
}};
...
(function(e,c,a,g){var d="gigyaSocialLogin",f={subAccountService:"//io.playstation.com/playstation/psn/issubaccount"};
function b(l,k){var h=this;
h.element=l;
h.$element=e(h.element);
h.options=e.extend({},f,k);
h._defaults=f;
h._name=d;
if(typeof gigya!=="undefined"){h.init()
}}b.prototype.init=function(){var h=this;
var k={version:2,showTermsLink:"false",hideGigyaLink:true,height:100,width:218,containerID:"gigyaLoginContainer",buttonsStyle:"fullLogoColored",autoDetectUserProviders:"",facepilePosition:"none"};
if(PDC.Profile.isLoggedIn()){e.ajax({url:h.options.subAccountService,data:{handle:PDC.Profile.getOnlineID()},dataType:"json",cache:false}).done(function(l){if(typeof l.isExists!=="undefined"&&l.isExists!==null&&l.isExists===false){gigya.socialize.showLoginUI(k);
gigya.socialize.addEventHandlers({onLogin:h.onLoginHandler,onLogout:h.onLogoutHandler});
e(c).on("socialLoginVerification",function(m,n){gigya.socialize.getUserInfo({callback:h.retrieveUserInfo})
})
}})
}};
b.prototype.checkLoginInit=function(){var h=this;
gigya.socialize.getUserInfo({callback:h.retrieveUserInfo})
};
b.prototype.retrieveUserInfo=function(l){var k=e(".CM156-competeConnectHeroBanner");
if(l.errorCode==0){k.$contentWrap=k.find(".gigya-addon");
k.$avatarBlock=k.find(".avatar-block");
k.$userInfo=k.find(".user-info");
var h=l.user;
if(h.isConnected){k.$contentWrap.addClass("hide");
if(k.$contentWrap.data("gigyafetchsocialname")==="true"||k.$contentWrap.data("gigyafetchsocialpic")==="true"){if(k.$contentWrap.data("gigyafetchsocialname")==="true"){var n=h.firstName;
if(n!==null){k.$userInfo.find("h2").text(n)
}}if(k.$contentWrap.data("gigyafetchsocialpic")==="true"){var m=h.thumbnailURL;
if(m!==""){k.$avatarBlock.find("img").attr("src",decodeURIComponent(m))
}}}}}};
b.prototype.onLogoutHandler=function(h){};
b.prototype.onLoginHandler=function(m){var k=e(".CM156-competeConnectHeroBanner");
try{k.$contentWrap=k.find(".gigya-addon");
k.$avatarBlock=k.find(".avatar-block");
k.$userInfo=k.find(".user-info");
k.$contentWrap.addClass("hide");
var h=m.user;
if(User!==""&&User.UID!==""){if(k.$contentWrap.data("gigyafetchsocialname")==="true"||k.$contentWrap.data("gigyafetchsocialpic")==="true"){if(k.$contentWrap.data("gigyafetchsocialname")==="true"){var o=User.firstName;
if(o!==null){k.$userInfo.find("h2").text(o)
}}if(k.$contentWrap.data("gigyafetchsocialpic")==="true"){var l=User.thumbnailURL;
if(l!==""){k.$avatarBlock.find("img").attr("src",decodeURIComponent(l))
}}}}}catch(n){}};
e.fn[d]=function(h){return this.each(function(){if(!e.data(this,"plugin_"+d)){var k=new b(this,h);
e.fn[d].refArray.push(k);
e.data(this,"plugin_"+d,k)
...
(function(f,d,a,h){var e="compareGameTrophies",g={friendListData:{},compareData:{},sortOptions:[{title:Adobe.I18n.get("Order of game"),value:"gameOrder"},{title:Adobe.I18n.get("Trophy Type"),value:"trophyType"},{title:Adobe.I18n.get("Trophies Owned"),value:"trophiesOwned"}],curSort:"gameOrder",tableColms:5,curPage:1,tilesPerPage:16,showMoreLink:true,colorBoxProps:{initialWidth:400,initialHeight:300,scrolling:true,width:"760px",height:"500px",maxWidth:"760px",maxHeight:"695px",overlayClose:true,photo:false}};
function c(l,k){this.element=l;
this.$element=f(this.element);
this.$elementParent=this.$element.parent();
...
PDC.MyPlaystationProfile.getSignedUserData(function(l){if(l.handle&&PDC.MyPlaystationProfile.isFriendsAuthExist(false)){k.loginUserId=l.handle;
...
d.sessionStorage.compareTrophiesVars=m;
k.initTrophiesCompare()
};
c.prototype.initTrophiesCompare=function(){var k=this;
var l=k.options;
if(f.cookie("compareTrophiesVars")){d.sessionStorage.compareTrophiesVars=decodeURIComponent(f.cookie("compareTrophiesVars"));
k.compareVars=k.encodeUrlVars(d.sessionStorage.compareTrophiesVars);
f.removeCookie("compareTrophiesVars",{domain:".playstation.com",path:"/"})
...
(function(e,c,a,g){var d="compareGameTrophiesGamesList",f={gameListDataUrl:"//my.playstation.com/playstation/psn/profile/user/gameList/",noGameListMsg:"What, no games played yet!",noGameIcon:"/etc/designs/pdc/clientlibs_base/images/CM162_CompateGamesTrophies/icon-recently-played-you.png"};
function b(k,h){this.element=k;
this.$element=e(this.element);
this.$elementParent=this.$element.parent();
this.$key=e(this.element).attr("id");
...
var k=h.options;
h.exploreGamesLink=h.$element.closest("section").find(".exploreGamesLink").data("href");
h.myTrophiesPageLink=h.$element.closest("section").find(".myTrophiesPageLink").data("href");
if(e.cookie("compareTrophiesVars")){c.sessionStorage.compareTrophiesVars=decodeURIComponent(e.cookie("compareTrophiesVars"));
h.compareVars=h.encodeUrlVars(c.sessionStorage.compareTrophiesVars);
e.removeCookie("compareTrophiesVars",{domain:".playstation.com",path:"/"})
}else{h.compareVars=h.encodeUrlVars(c.sessionStorage.compareTrophiesVars)
}h.curGameId=h.compareVars.gid;
if(!h.compareVars.gid){c.location=h.myTrophiesPageLink
}e(c).on("compareGameTrophiesLoaded",function(m,l){l=l||{};
h.buildGameListDropdown();
if(!l.gameId){c.location=h.myTrophiesPageLink
}});
...
h.gameListAjax=e.ajax({url:k.gameListDataUrl,cache:false,async:true,crossDomain:true,dataType:"json",data:{userInfoCookie:e.cookie("userinfo"),callback:"JSON_CALLBACK"}}).done(function(l){h.$element.find(".ajax-loader").remove();
k.gameListData=l;
if(l.list){h.sortGameList()
}h.curGameId=h.curGameId||l.curGameId;
...
e("<a>").attr({"class":"link-btn blue-btn   -btn",href:k.exploreGamesLink}).html('<div class="btn ">'+Adobe.I18n.get("Explore Games")+"</div>").appendTo(m)
};
...
var k=h.options;
var m=h.encodeUrlVars(c.sessionStorage.compareTrophiesVars);
var n="";
