...
PDC.Browser.getViewport=function(){return d.getComputedStyle(a.body,":after").content.replace(/"/g,"")
}
...
PDC.Region.Regions={};
PDC.Region.Regions.SCEA=["en-us","en-ca","fr-ca"];
...
if(m!=null){if(m.match("SCEA")&&a.cookie.match("userinfo=")&&a.cookie.match("ps_profile_US=")){e=true;
if(PDC.Header!=null&&PDC.Header.PostLoginAjaxUrl!=null&&PDC.Header.PostLoginAjaxUrl){var n=getCookie("userinfo");
c.ajax({type:"POST",url:PDC.Header.PostLoginAjaxUrl,data:"userInfoCookie="+n,success:function(){},error:function(){}})
}}else{if(m.match("BR")&&a.cookie.match("userinfo=")&&a.cookie.match("ps_profile_US=")){e=true
}else{if(m.match("LATAM")&&a.cookie.match("userinfo=")&&a.cookie.match("ps_profile_US=")){e=true;
...
setCookie("test",k)
}}else{if(m.match("SCEE")&&a.cookie.match("ps_profile=")){e=true
...
PDC.Profile.setProfile=function(){if(PDC.Profile.isLoggedIn()){var j=PDC.Region.getRegion();
var i=null;
if(j!=null){if(j.match("SCEA")||j.match("BR")||j.match("LATAM")){i="ps_profile_US"
}else{if(j.match("SCEE")){i="ps_profile"
}}if(i!=null){var e=getCookie(i);
if(e!=null){var n=decodeURIComponent(e);
var f=n.split("&");
for(var l in f){if(f.hasOwnProperty(l)){var g=f[l];
if(g!=null){var k=g.split("=");
if(k.length>1){PDC.addProperty("Profile",k[0],k[1])
}}}}}}var m="uh";
var h=getCookie(m);
if(h!=null){PDC.addProperty("Profile","encrypted_id",h)
}}}};
...
PDC.Profile.isMature=function(h){var l=null;
var i=PDC.Profile.isLoggedIn();
if(i){var k=PDC.Region.getRegion();
if(k!=null){if(k.match("SCEA")&&a.cookie.match("ag_US=")){var g=getCookie("ag_US");
if(g=="m"){l=true
}else{l=false
}}else{if(k.match("BR")&&a.cookie.match("ag_BR=")){var g=getCookie("ag_BR");
if(g=="m"){l=true
}else{l=false
}}else{if(k.match("LATAM")&&a.cookie.match("ag_MX=")){var g=getCookie("ag_MX");
if(g=="m"){l=true
}else{l=false
}}else{if(k.match("SCEE")){var j=PDC.Profile.getProfileProp("ar"),m=PDC.getAgeFromSequence(j);
if(!h){agt=PDC.Header.AgeGateThreshold
}else{agt=h
}if(m==-1){return null
}if(m&&agt){var e=parseInt(m);
var f=parseInt(agt);
return e>=f
}}}}}}}return l
}
...
var e=function(h){PDC.MyPlaystationProfile.signedUserData=h||{};
c(PDC.MyPlaystationProfile.callbacks).each(function(j,k){k(PDC.MyPlaystationProfile.signedUserData)
});
PDC.MyPlaystationProfile.callbacks=[];
PDC.MyPlaystationProfile.checkingUserSignIn=false
};
if(a.cookie.match("userinfo")){if(PDC.MyPlaystationProfile.signedUserData.handle){e(PDC.MyPlaystationProfile.signedUserData);
return 0
}if(PDC.MyPlaystationProfile.checkingUserSignIn){return 0
}if(PDC.Header.MyPlaystationCookieDecryptionUrl===""){e(null);
return 0
}PDC.MyPlaystationProfile.checkingUserSignIn=true;
var f={userInfoCookie:c.cookie("userinfo")};
c.ajax({type:"get",async:true,crossDomain:true,data:f,cache:false,url:PDC.Header.MyPlaystationCookieDecryptionUrl,dataType:"json"}).done(function(h){if(h.handle){if(!h.avatarUrl){h.avatarUrl="/etc/designs/pdc/clientlibs_base/images/CM156_CompeteConnectHeroBanner/defaultavatar.png"
}e(h)
}else{e(null)
}}).fail(function(j,i,h){e(null)
})
}else{e(null)
}};
PDC.MyPlaystationProfile.redirectToLogin=function(){var e=PDC.Locale.getLocale()[0];
a.location=PDC.Header.MyPlaystationLoginUrl+"?locale="+globalLocale+"&postSignInURL="+a.location.href+"&cancelUrl=http://www.playstation.com/"+e+"/"
};
...
(function(c,b,a,d){c(a).ready(function(){var l=c(this),j=c(".background-section"),k=false,i=false,e=c(b).width(),g=b.location.hash;
...
function f(){j.each(function(){var n=c(this),o=n.attr("data-alternative-background"),m=n.css("background-image");
if(o!==d){n.attr("data-alternative-background",m).css("backgroundImage",o)
}})
}if(e>=1600&&!k){j.addClass("upper-limit");
k=true
}if(e<=800&&!i){i=true;
f()
...
(function(d,c,b,e){function a(){var f=d("body,html");
f.animate({scrollTop:0},1000)
}d(".nav-link-top").live("click",function(f){f.preventDefault();
a()
});
d(".to-top").live("click",function(f){f.preventDefault();
a()
})
})(jQuery,window,document);
...
a.attr("onclick",'return PDC.SC.click("leadproductinfo", this, "button", null);');
a.attr("target","_self")
...
this.init()
}b.prototype.init=function(){console.log(this.result);
if(this.result){this.appendJSON(this.result,function(h){this.$ul.addGameTile(h)
});
...
(function(c){b.template='<div class="CM091-gametile game-tile x-sc" id="137122767921067" data-sc="{game_title: \'<%= productTitle %>\'}"><div class="tile clearfix"><a class="game-tile-image" href="<%= url %>" onclick="return PDC.SC.click(\'gametile\', this, \'image\', {linktitle_override: \'_title\', _title: \'<%= productTitle %>\'});"><img src="<%= img %>" alt=""></a><div class="game-tile-details"><span><%= platform%></span><h2><a class="title" href="<%= url %>"><%= title %></a></h2></div></div><div class="game-tile-hover"><div class="background"><a class="game-tile-image" href="<%= url %>" onclick="return PDC.SC.click(\'gametile\', this, \'image\', {linktitle_override: \'_title\', _title: \'<%= productTitle %>\'});"><% if (ageRatingImage && !ratingTbc) {%><div class="game-tile-hover-cert pegi-rating"><img src="<%= ageRatingImage %>" alt="<%= ageRatingImageAlt %>" title="<%= ageRatingImageAlt %>" /></div><%}%><img src="<%= hoverImg %>" alt=""></a><div class="game-tile-details clearfix"><span><%= platform %></span><h2><a class="title" href="<%= url %>" onclick="return PDC.SC.click(\'gametile\', this, \'text\');"><%= title %></a></h2><% if (buttonTxt) {if(buttonType.indexOf("auto-retail") >= 0 && typeof PriceSpider !== "undefined"){%><div class="priceSpider" id="<%=upc%>" data-text="<%=buttonTxt%>"></div><%}else{%><a id="" target="<%= buttonTarget%>" class="link-btn <%= buttonType%> <%= buttonColor%> small-btn x-sku-<%= sku%> x-title-<%= productTitle%>" x-upc-<%= upc %> style="" <% if(!newModal) {%> href="javascript:void(0);" data-href="<%= buttonURL%>" <% } else {%> href="<%= buttonURL%>" <% }%>onclick="return PDC.SC.click(\'gametile\', this, \'button\', {link_section_name: \'<%= productTitle %>\', <% if(!newModal) {%>open: $(this).data(\'href\'), <%}%>newWindow: <%= newWindow %>});"><div class="btn "><%= buttonTxt %></div></a><%}%><%}%><h3 class="clearfix"><%= releaseDate %></h3>                   </div></div></div></div>';
...
b.prototype.replace=function(){if(typeof this.platform=="undefined"||this.platform==null){this.platform=""
}this.jsondata={title:this.title,img:this.getImageURL(),hoverImg:this.getHoverImageURL(),platform:this.platform.replace(/(\w),(\w)/g,"$1, $2"),description:this.description,url:this.url,buttonColor:this.buttonColor,buttonTxt:this.buttonTxt,buttonURL:this.buttonURL,buttonType:this.buttonType,ageRatingImage:this.getAgeRatingImageURL(),ageRatingImageAlt:this.ageRatingImageAlt,ratingTbc:this.ratingTbc,releaseDate:this.getDate(),newWindow:this.buttonURL.indexOf("https://store")==0,newModal:this.buttonType.indexOf("lb_iframe")>0,buttonTarget:this.buttonURL.indexOf("https://store")==0?"_target":"_self",productTitle:this.getEscapedTitle(),sku:this.sku,upc:this.upc};
var d=_.template(b.template,this.jsondata);
return d
};
...
}function g(l){if(l==null){return null
}var n=Date.now()-new Date(l).getTime();
var m=new Date(n);
return Math.abs(m.getUTCFullYear()-1970)
...
PDC.SC.click("agegate",j,"button",{events:["event39"]})
...
PDC.SC.click("agegate",j,"button",{events:["event40"]})
}p.find(".day, .month, .year").setAgeGateInputEvents(i);
i.setAgeGateSubmitHandler(o.options.expired,o.options.threshold,m,n);
e(c).on("resize.ageGate",function(){var s=e(c).height(),r=e("#main-header nav").outerHeight(),u=r+60,q=e(a.getElementById("age-gate-overlay"));
...
(function(g){var o="GamePaths",k="GameTitles",i="GameDescs",c="GameDates",b="ImageScene7Paths",h=",",e=";",j="SmallHero_Hygiene_Legal_09Oct13",d={wwwdev:"//media.playstation.com/is/image/SCEADEV/",wwwqa:"//media.playstation.com/is/image/SCEAQA/",wwwstage:"//media.playstation.com/is/image/SCEASTAGE/",wwwuat:"//media.playstation.com/is/image/psmedia/",www:"//media.playstation.com/is/image/SCEA/"},n={path:[],title:[],desc:[],date:[],scene7Path:[]};
...
if(typeof C==="undefined"||C===null||C===""){g(D).on("click",function(){return PDC.SC.click("threecolfeature",this,"image",{open:g(this).data("href"),openTarget:"_self"})
})
}if(H==="undefined"){H="//media.playstation.com/is/image/SCEA/playstation-now-spotlight-27jun14?$ThreeColFeature_Image$"
...
a(document).ready(function(){console.log("before reSizeWin()");
a.fn.reSizeWin=function(){window.resizeTo(screen.width,screen.height);
window.moveTo(0,0);
console.log("inside reSizeWin()")
};
if(PDC.Browser.device.match(/ps3/i)||PDC.Browser.device.match(/ps4/i)){a.fn.reSizeWin();
console.log("after reSizeWin()");
