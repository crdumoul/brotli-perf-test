var skaTools = {
	...
	main: function(){
		...
		if(typeof globalNavDefaultSelections.tGuardSKADown !="undefined" && globalNavDefaultSelections.tGuardSKADown == "false" )
		{	
			if(typeof skaTools.GNSessionCookie.UG != "undefined")
			{
				var UGDetails  = skaTools.GNSessionCookie.UG;
				for(var i=0; i < UGDetails.length; i ++){
					var UGDetail = UGDetails[i];				
					if(UGDetail.toLowerCase() == 'auth'){
						skaTools.hitURL(globalNavDefaultSelections.tGuardSKAURL);
					}
				}
...
if (document.addEventListener) {
	window.addEventListener("load", skaTools.main, false )
} else if (document.attachEvent) {
	window.attachEvent("onload", skaTools.main)
}