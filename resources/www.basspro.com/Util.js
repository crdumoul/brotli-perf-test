...
//This javascript function is used by the 'Add to Wish List' from drop down PDP page
function DDPDPAdd2WishList(orderitmform,wlistform)
{
       if (!busy) {
              busy = true;
              wlistform.action="/webapp/wcs/stores/servlet/InterestItemAdd";
              wlistform.catEntryId_0.value=orderitmform.catEntryId_0.value;
              wlistform.quantity_0.value=1;
              wlistform.URL.value='InterestItemDisplay';
              wlistform.submit();
       }
}


//This javascript function is used by the 'Multiple items Add to Wish List' button to set appropriate values before the form is submitted
//Quantity is hard-coded to value = 1 if input value of quantity other than greater than , since adding to wish list does not require a valid quantity.

function Add2MultiWishList(form)
{
  var inputsArr = form.getElementsByTagName("input");
    ...
    for(i in inputsArr)
    {
      ...
      if ( inputID != null && inputID.indexOf("quantity") != -1 ) {

        if(inputsArr[i].value > 0)
        {
         inputsArr[i].value = 1;
        }
