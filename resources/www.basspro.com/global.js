...
function loadTopNavigation() {
    ...
    if(tabval=="tab1-nav"){
      ...
      $("#sublevel-bar").css("background-color","#343434");
        ...
        $("#sublevel-bar").css({"background-color":"#990000","height":"6px"});
    }
        // Stores
                else if (tabval=="tab3-nav"){
        $("#sublevel-bar").css({"background-color":"#67604c","height":"6px"});
      ...
      $("#sublevel-bar").css({"background-color":"#240805","height":"6px"});
    }
        // NYI (not yet implemented)
        else if (tabval=="tab6-nav"){
      $("#sublevel-bar").css("background-color","#354b1a");
...
function loop(x){
  if (x<0){x=totalScenes-1};
  if (x>totalScenes-1){x=0};
  $("#output").html(x);


  $("div#cotw"+x).fadeIn(300).fadeTo(3000,1).fadeOut(300,function(){
    loop(x+1);
  });//end of div#s
  ...
  $(".cotw a.entire-div").mouseenter(function() {
    $(".cotw p.cotw-important").stop(true, true).slideDown();
  });
...
function makeLinksNonSecured() {
    ...
    var hostQualifier =  "http://"+ hostName ;
      if(hostQualifier.indexOf("basspro.com")==-1)
      {
       hostQualifier = mainhostname;
      }
    var tempvar = "";
     ...
     $("a").each(function(i) {
      if($(this).attr("href") === undefined || $(this).attr("href") == "")
      {
        ...
        else if ( (  /^(http|ftp|https):\/\/.*$/i.test( $(this).attr("href") ) )
             || ( /^mailto:.*$/i.test($(this).attr("href")) )
                || ( /^(javascript).*$/i.test( $(this).attr("href") ) || (($(this).attr("href") !== undefined) && $(this).attr("href").replace(/^\s+|\s+$/, '').indexOf('#' ) == 0 )))
        {
        // Don't do any thing
        }
        else if ( ($(this).attr("href") !== undefined) && ($(this).attr("href").indexOf("/") == 0))
        {
