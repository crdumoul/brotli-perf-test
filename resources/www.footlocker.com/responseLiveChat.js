...
//  Modified 4/18/2014:                        //
//  By Justin Pearce (Chrome Issues on popup)  //
//                                             //
...
var gChatWindowProperties = "width=" + gChatWindowWidth + ",height=" + gChatWindowHeight + ",menubar=yes,location=no,directories=no,status=yes,toolbar=no,scrollbars=auto,resizable=no,screenX=10,screenY=10,left=10,top=10";
var gHTTP = "http://";
var gHTTPS = "https://";
...
{
    //JP: Updated to address Chrome issues with onerror handling
	var testImg = new Image();
	if(!isIE()){
		testImg.onload = function(){ if(testImg.complete || testImg.readyState === 4){ createAndSubmitForm(); } }
	}else{
		createAndSubmitForm();
	}
	testImg.onerror = function(evt, url, line){ handleChatNoSSL(iChannel, serverName); }
