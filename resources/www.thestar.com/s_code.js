...
/*Insert the legacy.com URL that reports to productions rsid */
sc_legacyURL = "www.legacy.com";

/* star-branded third-party pages */
sc_thirdpartiesURL = "www3.thestar.com";

/*Insert the starclassifieds.com URL that reports to productions rsid */
sc_classifiedsURL = "www.starclassifieds.com";

/*Insert the mediakit URL that reports to productions rsid */
sc_mediakitURL = "mediakit.thestar.ca";

/*Insert the archive search URL that reports to productions rsid */
sc_archivesURL = "pqasb.pqarchiver.com/thestar";

/*Insert the base preproduction URL (e.g. www.yourdomain.com) */
sc_preprodURL="preprod.thestar.com";

/*Insert the desktop production report suite ID */
sc_desktop_prodRSID = "thestardesktop";

/*Insert the desktop dev report suite ID */
sc_desktop_devRSID = "thestardesktopdev";
...
function containsItemFromList(inputString, items){
    for(i = 0; i < items.length; i++){
        if(inputString.indexOf(items[i])!= -1){
            return true;
        }
    }
    return false;
...
if ('reportSuite' in document){
    s_account = document.reportSuite; // this comes from AEM
}
else if (containsItemFromList(tloc, production_URLs)){
    //Desktop Production
    s_account=sc_desktop_prodRSID;

} else{
    //Desktop Development
    s_account=sc_desktop_devRSID;
}
...
s.charSet = sc_charSet;//"ISO-8859-1";
s.currencyCode = sc_currency;//"CAD";
s.trackDownloadLinks = false;
s.trackExternalLinks = false;
s.trackInlineStats = true;
s.linkDownloadFileTypes = "exe,zip,wav,mp3,mov,mpg,avi,wmv,doc,pdf,xls";
s.linkInternalFilters = "javascript:,parentcentral.ca,thestar.com,yourhome.ca,healthzone.ca,thestar.blogs.com";
...
function s_doPlugins(s) {
    ...
    s.prop1 = s.eVar1 = "s_code_v3.0 - 2014-10-20";
    // Page URL
    s.prop32 = s.eVar32 = s.wd.location;
    /* Hierachy, Channel, Prop population */
    if(s.pageName){
        s.pageName = s.pageName.toLowerCase().replace(".html","");
        s.hierDelim="|";
    ...
    if(!s.pageName) {
        ...
        if(s.pageName.indexOf("topic") != -1){
            ...
            s.pageName=name;
        	s.prop12=s.eVar12="topic"; //Content Type
        }
        s.pageName = s.pageName.toLowerCase().replace(".html","");
        s.hier1=s.pageName.replace(/:/g,',');
        s.hps=s.split(s.hier1,",");
    ...
    if(!s.server){
        s.server=parent.window.location.host.replace(/www[0-9]*\./, '');
    }

    //Photogallery URL string
    if(s.prop12 == "slideshow"){
        s.prop18 = s.eVar18 = s.pageName.replace(/:/g,'|');
    }


    if (localStorage.getItem("userId") !== null && localStorage.getItem("userId") !== "null"){
        s.prop21 = s.eVar21 = localStorage.getItem("userId");
    }

    if (localStorage.getItem("signedIn") == "true"){
        s.prop22 = s.eVar22 = 'Logged In';
        if (localStorage.getItem("userLoc") !== null && localStorage.getItem("userLoc") !== "null"){
            s.prop23 = s.eVar23 = localStorage.getItem("userLoc");
        }
    } else{
        s.prop22 = s.eVar22 = 'Logged Out';
        s.prop23 = s.eVar23 = "";
        s.prop21 = s.eVar21 = "";
    }
    ...
    s.prop27 = s.eVar27 = hours + ":" + mins + " " + ampm;
    s.prop28 = s.eVar28 = days[now.getDay()];

    // Percent of page viewed
    s.prop29=s.eVar29=s.getPercentPageViewed();

    // Get Previous Page
    s.prop30=s.eVar30=s.getPreviousValue(s.pageName,'gpv_pn','');

    //Display ad category - section user was on when display ads served
    s.prop40 = s.eVar40 = s.prop2;

    //Below: Deprecated tracking
    ...
    // Internal search
    // if(!s.prop36) s.prop36=s.getQueryParam('q');
    // if(s.prop36) s.prop36=s.getValOnce(s.prop36,'s_gvo_c36',0);
    // if(s.prop36){
    //     s.pev2 = "searchresult";
    //     s.pageName = s.channel = s.hier1 = "search";
    //     s.eVar36=s.prop36=s.prop36.toLowerCase();
    //     s.prop40 = s.eVar40 = "search";
    ...
    //if(s.campaign) s.events=s.apl(s.events,"event9",",",2);

    /* Internal campaign tracking*/
    //if(!s.eVar43) s.eVar43=s.getQueryParam('icid');
    //s.eVar43=s.getValOnce(s.eVar43,'s_gvo_v43',0);
    //if(s.eVar43) s.events=s.apl(s.events,"event8",",",2);
...
//s.loadModule("Media")
//s.Media.autoTrack = false;//true;
//s.Media.trackWhilePlaying = true;
//s.Media.trackVars = "None";
//s.Media.trackEvents = "None";
//s.Media.trackVars = "events";
//s.Media.trackEvents = "event21,event22,event23";

//s.Media.trackSeconds = 15;
//s.Media.trackUsingContextData = true;
//s.Media.contextDataMapping = {
//   "a.media.name": "eVar39,prop39",
//  "a.media.segment": "eVar39",
//    "a.contentType": "eVar7",
//    "a.media.timePlayed": "event6",
...
s.trackingServer = window.trackingServer || "n.thestar.com";
s.trackingServerSecure = window.trackingServerSecure || "s.thestar.com";
